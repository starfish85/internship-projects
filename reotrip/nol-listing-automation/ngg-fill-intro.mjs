...
const DRAFT = process.env.NGG_DRAFT || readFileSync(join(__dir, '.ngg-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/nagoya-ghibli';
const IMAGES = ['ngg-1.jpg', 'ngg-2.jpg', 'ngg-3.jpg']
...
const HIGHLIGHT = [
  ...
  '나고야·지브리 파크 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${CITY}(나고야 / Nagoya)과 ${DEST}(지브리 파크 / Ghibli Park, 愛知·長久手) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 나고야 시내 호텔명·주소, 지브리 파크 픽업·샌딩 장소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

스튜디오 지브리 팬, 가족 여행, 소규모 그룹 등 나고야 시내에서 지브리 파크를 오가시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 나고야 시내 호텔 주소와 지브리 파크 픽업/샌딩 장소·이용 시간을 정확히 입력해 주세요.
...
const textOk = await page.evaluate(() => ({
  hl: /나고야|지브리|Nagoya|Ghibli/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /나고야|지브리|Nagoya|Ghibli|吉卜力|ジブリ/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Nagoya-Ghibli');
