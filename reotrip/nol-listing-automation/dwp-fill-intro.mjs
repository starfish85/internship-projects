/**
 * WSK 介绍：韩文文案 + 썸네일 3 图 → 保存然后
 * DRAFT d6615900-b0a5-4cfd-8342-6b3a69bfe8b4
 */
import fs from 'fs';
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/disney-wusongkou-port';
const IMAGES = ['dwp-1.jpg', 'dwp-2.jpg', 'dwp-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter(fs.existsSync);
const CITY = '상하이 디즈니랜드';
const DEST = '상하이 우숭커우 국제크루즈터미널';
const HEADLINE = `${CITY} ↔ ${DEST} 편도 전용 차량으로 여유로운 이동을 즐기세요!`;
const HIGHLIGHT = [
  `${CITY}과 ${DEST}을 편안하게 연결하는 단독 차량 이동`,
  '크루즈 출항/입항 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 차량 · 최대 4인 + 수하물 4개(24인치 이하)',
].join('\n');
const INTRO = `이 서비스는 ${CITY}과 ${DEST}(上海吴淞口国际邮轮港 / Shanghai Disney Resort ↔ Shanghai Wusongkou International Cruise Terminal) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 선박명·승하선 정보, 상하이 디즈니랜드명/주소, 픽업 장소, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 소규모 그룹 등 상하이 우숭커우 국제크루즈터미널을 이용하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 4개까지 적재 가능. 차량 공간을 고려해 적합한 차형을 선택해 주세요.

2.미팅 장소: 디즈니랜드 픽업 시 기사님이 지정 미팅 장소에서 픽업해 드립니다. 동일한 이름의 호텔이 있을 수 있으므로 호텔 이름과 주소를 함께 제공해 주세요. 공항 픽업 시 터미널/승하선 장소를 정확히 입력해 주세요.
...
3.예약 시 호텔명·주소, 항만 승하차 장소, 선박명 및 승하선 정보, 이용 시간, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('dwp-intro');
...
const textOk = await page.evaluate(() => ({
  hl: (document.querySelector('#headline, input[name=headline]')?.value || '').includes('우숭커우'),
  desc: (document.querySelector('#description, textarea[name=description]')?.value || '').includes('Wusongkou'),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing WSK');
