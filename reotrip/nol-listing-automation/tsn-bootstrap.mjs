/**
 * 东京站-成田机场 — 创建或恢复草稿
 * Excel (9) 日本接送 Excel(10) R103–106 · 7/10 售价1241/1603 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '도쿄역 ↔ 나리타 공항(NRT) 단독 차량 편도 이동 서비스';
const INTERNAL = '东京站-成田机场';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 东京站-成田机场 草稿');
killPeerCdpScripts('tsn-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /도쿄역.*나리타|东京站-成田机场/i.test(body)) {
  console.log('【将要】恢复草稿 东京站-成田机场');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /东京站-成田机场|도쿄역 ↔ 나리타/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.tsn-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.tsn-draft-id', import.meta.url), m[1]);
process.exit(0);
