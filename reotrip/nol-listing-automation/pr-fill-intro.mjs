...
const DRAFT = '92333d3b-0131-4ed5-9641-3c0ed2e87ff1';
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/pvg-railway';
const IMAGES = ['pr-1.jpg', 'pr-2.jpg', 'pr-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter(fs.existsSync);
const CITY = '상하이 푸동국제공항(PVG)';
const DEST = '상하이역';
...
const INTRO = `이 서비스는 ${CITY}과 ${DEST}(上海浦东国际机场 / Shanghai Pudong International Airport ↔ Shanghai Railway Station) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명·도착/출발 시간, 상하이역 출구/미팅 장소, 픽업 장소, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 소규모 그룹 등 상하이 푸동국제공항을 이용하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 4개까지 적재 가능. 차량 공간을 고려해 적합한 차형을 선택해 주세요.

2.미팅 장소: 공항 픽업 시 터미널/출구 정보를 정확히 입력해 주세요. 역 픽업 시 출구/만남의 장소를 정확히 입력해 주세요.
...
const textOk = await page.evaluate(() => ({
  hl: (document.querySelector('#headline, input[name=headline]')?.value || '').includes('푸동'),
  desc: (document.querySelector('#description, textarea[name=description]')?.value || '').includes('Pudong'),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing PVG');
