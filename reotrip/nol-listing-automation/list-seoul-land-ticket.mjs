/**
 * Seoul Land Admission Ticket
 * CSV 韩国门票产品 R6: 一日票 · 只卖成人 · 外籍特惠（不支持韩国国籍）
 * 【售价来源】售价hkd=141（表显示；加价逻辑 113 未用；未自算）skill 0h
 * 单选项 1일 입장권；0n 双门禁 代表+按数量
 * 串行：本脚本只上 Seoul Land，禁并行其他
 * Never 提交审核
 *
 * usage: node list-seoul-land-ticket.mjs [bootstrap|attrs|intro|regs|options]
 */
...
const DRAFT_FILE = join(__dir, '.seoul-land-draft-id');
const PHASE = process.argv[2] || 'bootstrap';
// 【售价来源】售价hkd=252（CSV 韩国门票 R6 表显示；加价逻辑=113 未用；未自算）
const PRICE = (process.env.SEOUL_LAND_PRICE || process.argv[3] || '141').trim();
const PRODUCT_KO = '서울랜드 1일 입장권 (외국인 특혜)';
const INTERNAL = 'Seoul Land 1日票 外籍特惠';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/seoul-land';
const IMAGES = ['sl-1.jpg', 'sl-2.jpg', 'sl-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '서울랜드에서 하루 종일 놀이동산을 즐기세요!';
const HIGHLIGHT = [
  '서울랜드 1일 입장권 · 성인 전용 · 외국인 특혜 요금',
  '과천 테마파크 종일 이용 · 이메일 전자티켓/이용 안내',
  '한국 국적자 구매 불가 · 예약 시 전화·이메일·여권 정보 필수',
].join('\n');
const INTRO = `서울랜드(Seoul Land) 1일 입장권입니다.
경기 과천의 대표 테마파크에서 다양한 어트랙션을 하루 종일 즐길 수 있습니다.
...
가족·친구와 테마파크를 즐기고 싶은 외국인 이용객께 추천합니다.`;
...
function readDraft() {
  if (process.env.SEOUL_LAND_DRAFT) return process.env.SEOUL_LAND_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=113 未用作售价；未自算）');
  ...
  // resume seoul land only
  if (/서울랜드|Seoul Land|Seoulland/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /서울랜드|Seoul Land|Seoulland/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /서울랜드|Seoul Land|과천|Gwacheon/.test(t)
...
async function attrs(page) {
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 서울랜드 / Seoul Land');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['Seoul Land', '서울랜드', 'Seoulland', '과천 서울랜드'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const score = (s) => {
            if (/Hotel|호텔|酒店|숙박|住宿/i.test(s) && !/서울랜드|Seoul Land/i.test(s)) return -50;
            if (/广州|廣州|北京|上海|Macau|에버랜드|Everland|남이섬|롯데월드(?!)/i.test(s)) return -50;
            if (/서울랜드|Seoul Land|Seoulland/i.test(s) && /과천|Gwacheon|경기|Gyeonggi|韩国|Korea|서울|Seoul/i.test(s)) return 100;
            if (/서울랜드|Seoul Land|Seoulland/i.test(s)) return 85;
            return 0;
          };
          let best = null;
          let bs = 0;
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '서울랜드 1일 입장권' }],
...
async function options(page) {
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('서울랜드 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
