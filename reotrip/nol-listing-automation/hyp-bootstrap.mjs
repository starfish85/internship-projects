/**
 * 羽田机场-横滨港 — 创建或恢复草稿
 * Excel (9) 日本接送 R78–81 · 7/10 成本737.25/972.55 → 售价1053/1389 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '하네다 공항(HND) ↔ 요코하마항 단독 차량 편도 이동 서비스';
const INTERNAL = '羽田机场-横滨港';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 羽田机场-横滨港 草稿');
killPeerCdpScripts('hyp-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /하네다.*요코하마|羽田机场-横滨港/i.test(body)) {
  console.log('【将要】恢复草稿 羽田机场-横滨港');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /羽田机场-横滨港|하네다 공항.*요코하마항/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.hyp-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.hyp-draft-id', import.meta.url), m[1]);
process.exit(0);
