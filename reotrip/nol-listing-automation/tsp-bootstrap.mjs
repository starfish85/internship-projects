/**
 * 东京站-东京港 — 创建或恢复草稿
 * Excel(11) R156–159 · 7/10 成本→售价784/1176 · 港口无航班 日本接送 R86–89 · 7/10 成本737.25/972.55 → 售价1053/1389 · 无假日
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '도쿄역 ↔ 도쿄항 단독 차량 편도 이동 서비스';
const INTERNAL = '东京站-东京港';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 东京站-东京港 草稿');
killPeerCdpScripts('tsp-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /도쿄역.*도쿄항|东京站-东京港/i.test(body)) {
  console.log('【将要】恢复草稿 东京站-东京港');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /东京站-东京港|도쿄역 ↔ 도쿄항/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.tsp-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.tsp-draft-id', import.meta.url), m[1]);
process.exit(0);
