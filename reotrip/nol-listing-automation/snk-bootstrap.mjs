/**
 * 札幌市区-二世谷 — 创建或恢复草稿
 * Excel(11) 日本接送 R140–143 · 7/10 售价3476/3950 · 无假日 · 城际无航班 · 札幌-二世谷
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '삿포로 시내 ↔ 니세코 단독 차량 편도 이동 서비스';
const INTERNAL = '札幌市区-二世谷';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 札幌市区-二世谷 草稿');
killPeerCdpScripts('snk-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /삿포로 시내.*니세코|札幌市区-二世谷/i.test(body)) {
  console.log('【将要】恢复草稿 札幌市区-二世谷');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /札幌市区-二世谷|삿포로 시내 ↔ 니세코/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.snk-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.snk-draft-id', import.meta.url), m[1]);
process.exit(0);
