/**
 * SHA 介绍：韩文文案 + 썸네일 3 图 → 保存然后
 * DRAFT 761d6911-b55e-47c0-af96-08f04636f8a2
 */
import fs from 'fs';
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/shanghai-hongqiao-airport';
const IMAGES = ['sha-1.jpg', 'sha-2.jpg', 'sha-3.jpg']
...
const INTRO = `이 서비스는 ${CITY}과 ${DEST}(上海虹桥国际机场 / Shanghai Hongqiao International Airport) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명, 도착/출발 시간, 상하이 시내 호텔명/주소, 픽업 장소, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 소규모 그룹 등 상하이 훙차오국제공항을 이용하시는 분들께 적합합니다.`;
...
const textOk = await page.evaluate(() => ({
  hl: (document.querySelector('#headline, input[name=headline]')?.value || '').includes('훙차오'),
  desc: (document.querySelector('#description, textarea[name=description]')?.value || '').includes('Hongqiao'),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing SHA');
