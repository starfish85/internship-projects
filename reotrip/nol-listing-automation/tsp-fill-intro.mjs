...
const DRAFT = process.env.TSP_DRAFT || readFileSync(join(__dir, '.tsp-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/tokyo-stn-port';
const IMAGES = ['tsp-1.jpg', 'tsp-2.jpg', 'tsp-3.jpg']
...
const INTRO = `이 서비스는 ${CITY}(도쿄역 / Tokyo Station)과 ${DEST}(도쿄항 / Tokyo Port · 東京国際クルーズターミナル 등) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 도쿄역 승하차 위치, 도쿄항(크루즈 터미널) 픽업/샌딩 장소, 이용 시간, 연락 가능한 휴대전화 번호, 선박명 및 승하선 정보, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 크루즈/페리 이용 등 도쿄역과 도쿄항을 오가시는 분들께 적합합니다.`;
...
2.미팅 장소: 도쿄역 승하차 위치와 도쿄항 픽업/샌딩 장소·이용 시간, 선박명 및 승하선 정보를 정확히 입력해 주세요.
...
3.예약 시 호텔 주소, 도쿄항 픽업/샌딩 장소, 선박명·승하선 시간, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('tsp-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /도쿄|Yokohama/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /도쿄|Yokohama/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Yokohama');
