/**
 * 成田机场(NRT)-东京迪士尼 — 创建或恢复草稿
 * Excel(11) 日本接送 R188–191 · 7/10 成本→售价1255/1961 · 机场+迪士尼 航班+입장권
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '나리타 공항(NRT) ↔ 도쿄 디즈니 리조트 단독 차량 편도 이동 서비스';
const INTERNAL = '成田机场(NRT)-东京迪士尼';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 成田机场(NRT)-东京迪士尼 草稿');
killPeerCdpScripts('ntd-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /나리타.*디즈니|成田机场\(NRT\)-东京迪士尼|成田.*迪士尼|NRT.*Disney/i.test(body)) {
  console.log('【将要】恢复草稿 成田机场(NRT)-东京迪士尼');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /成田机场\(NRT\)-东京迪士尼|成田.*迪士尼|나리타 공항\(NRT\) ↔ 도쿄 디즈니/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.ntd-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.ntd-draft-id', import.meta.url), m[1]);
process.exit(0);
