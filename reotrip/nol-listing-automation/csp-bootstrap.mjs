/**
 * 新千岁机场(CTS)-札幌酒店 — 创建或恢复草稿
 * Excel(11) R152–155 · 7/10 售价2246/3002 · 机场航班预约 日本接送 Excel(10) R107–110 · 7/10 售价1580/2054 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '신치토세 공항(CTS) ↔ 삿포로 시내 호텔 단독 차량 편도 이동 서비스';
const INTERNAL = '新千岁机场(CTS)-札幌酒店';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 新千岁机场(CTS)-札幌酒店 草稿');
killPeerCdpScripts('csp-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /신치토세.*삿포로|新千岁机场\\(CTS\\)-札幌|新千岁.*札幌/i.test(body)) {
  console.log('【将要】恢复草稿 新千岁机场(CTS)-札幌酒店');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /新千岁机场\\(CTS\\)-札幌|新千岁.*札幌|신치토세 공항\\(CTS\\) ↔ 삿포로/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.csp-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.csp-draft-id', import.meta.url), m[1]);
process.exit(0);
