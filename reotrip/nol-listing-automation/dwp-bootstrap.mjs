...
const PRODUCT_KO =
  '상하이 디즈니랜드 ↔ 상하이 우숭커우 국제크루즈터미널 단독 차량 편도 이동 서비스';
const INTERNAL = '上海迪士尼-吴淞口港口';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 WSK 草稿');
killPeerCdpScripts('dwp-bootstrap');
...
if (
  ...
  /디즈니|迪士尼|Disney|우숭커우|吴淞口|크루즈/.test(body)
) {
  console.log('【将要】恢复草稿 迪士尼-吴淞口（排除吴淞口/火车站）');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /우숭커우|吴淞口|WSK|크루즈|邮轮|上海迪士尼-吴淞口港口/ })
    .filter({ hasNotText: /시내 호텔|市区酒店|虹桥|浦东|PVG|SHA|火车站/ })
    .first();
  if (await card.count()) {
