...
const DRAFT = process.env.CFU_DRAFT || readFileSync(join(__dir, '.cfu-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/cts-furano';
const IMAGES = ['cfu-1.jpg', 'cfu-2.jpg', 'cfu-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '신치토세 공항(CTS)';
const B = '후라노';
const HEADLINE = `${A} ↔ ${B} 편도 전용 차량으로 여유로운 이동을 즐기세요!`;
const HIGHLIGHT = [
  `${A}과 ${B}를 편안하게 연결하는 단독 차량 이동`,
  '항공편·후라노 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${A}(New Chitose Airport)와 ${B}(후라노 / Furano, 홋카이도) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명·도착/출발 시간·터미널, 후라노 호텔·숙소·픽업 장소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

신치토세 공항 출도착 후 후라노(라벤더·스키·온천)로 바로 이동하시거나, 관광 후 공항으로 이동하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 항공편·터미널 정보와 후라노 호텔·픽업 장소, 이용 시간을 정확히 입력해 주세요.
...
3.예약 시 항공편명, 도착 또는 출발 시간, 터미널, 후라노 주소, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('cfu-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /신치토세|CTS|후라노|Furano|Chitose/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /신치토세|CTS|후라노|Furano|Chitose|Hokkaido/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Chitose-Kyoto');
