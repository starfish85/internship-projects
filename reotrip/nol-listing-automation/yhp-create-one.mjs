/**
 * 横滨市区酒店-横滨港 创建单个选项（无时段）
 * usage: node yhp-create-one.mjs 7go|10go|7rtn|10rtn
 * Excel 成本/0.7 half-up: 7→1333  10→1658
 */
...
const MAP = {
  ...
  '10go': {
    ...
    desc: `${HOTEL} 출발 → ${DEST} 도착 편도 이동 (10인승 차량, 최대 9인 탑승 가능)\n26인치 이하 수하물 기준: 최대 10개까지 적재 가능\n별도 선박 티켓, 아동용 카시트, 야간 할증은 포함되어 있지 않습니다.`,
    pt: '10인승 가는',
    ptd: '10인승 차량',
    price: '1389',
  },
  '7rtn': {
    ...
    desc: `${DEST} 출발 → ${HOTEL} 도착 편도 이동 (7인승 차량, 최대 4인 탑승 가능)\n24인치 이하 수하물 기준: 최대 5개까지 적재 가능\n별도 선박 티켓, 아동용 카시트, 야간 할증은 포함되어 있지 않습니다.`,
    pt: '7인승 오는',
    ptd: '7인승 차량',
    price: '1053',
  },
  '10rtn': {
    ...
    desc: `${DEST} 출발 → ${HOTEL} 도착 편도 이동 (10인승 차량, 최대 9인 탑승 가능)\n26인치 이하 수하물 기준: 최대 10개까지 적재 가능\n별도 선박 티켓, 아동용 카시트, 야간 할증은 포함되어 있지 않습니다.`,
    pt: '10인승 오는',
    ptd: '10인승 차량',
    price: '1389',
  },
...
const OPT = MAP[key];
if (!OPT) {
  console.error('usage: node yhp-create-one.mjs 7go|10go|7rtn|10rtn');
  process.exit(2);
}

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
killPeerCdpScripts('yhp-create-one');
