/**
 * TeamLab SuperNature Macao Admission Ticket
 * CSV 国内景区门票 (3) R46: 1日票 · 只卖成人 · 无需护照号
 * 【售价来源】售价hkd=303（表显示；加价逻辑 211.8 未用；未自算）skill 0h
 * 单选项 1일 입장권；代表预约 电话+邮箱 only（表：无需护照号）
 * Never 提交审核
 *
 * usage: node list-teamlab-macao-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 */
...
const DRAFT_FILE = join(__dir, '.teamlab-macao-draft-id');
const PHASE = process.argv[2] || 'all';
// 【售价来源】售价hkd=303（CSV (3) R46 表显示；加价逻辑=211.8 未用；未自算）
const PRICE = (process.env.TEAMLAB_MACAO_PRICE || process.argv[3] || '303').trim();
const PRODUCT_KO = '마카오 팀랩 슈퍼네이처 1일 입장권';
const INTERNAL = 'TeamLab SuperNature Macao Admission Ticket';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/teamlab-macao';
const IMAGES = ['tl-1.jpg', 'tl-2.jpg', 'tl-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '마카오 팀랩 슈퍼네이처 디지털 아트를 체험하세요!';
const HIGHLIGHT = [
  '팀랩 슈퍼네이처 마카오 1일 입장권 · 성인 전용',
  '몰입형 디지털 아트 전시 · 이메일 전자티켓/이용 안내',
  '예약 시 전화·이메일 필수 (여권번호 불필요)',
].join('\n');
const INTRO = `마카오 팀랩 슈퍼네이처(teamLab SuperNature Macao) 1일 입장권입니다.
몰입형 디지털 아트 공간을 자유롭게 관람할 수 있습니다.

포함 사항:
- 팀랩 슈퍼네이처 마카오 1일 입장권 1매 (성인)
...
function readDraft() {
  if (process.env.TEAMLAB_MACAO_DRAFT) return process.env.TEAMLAB_MACAO_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=211.8 未用作售价；未自算）');
  ...
  if (/팀랩 슈퍼네이처|teamLab SuperNature Macao|澳门塔|澳門塔|전망대 1일/i.test(body) && !/TeamLab|팀랩|teamlab/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /팀랩 슈퍼네이처|teamLab SuperNature Macao|澳门塔|澳門塔/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /澳门|澳門|Macau|Macao|마카오|TeamLab|teamlab|팀랩|SuperNature/.test(t)
...
async function attrs(page) {
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 澳门塔 / teamLab SuperNature Macao');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['teamLab SuperNature Macao', '澳门塔', '澳門塔', '팀랩 슈퍼네이처', 'Macao Tower'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const hit =
            items.find((e) => {
              const s = e.innerText || '';
              if (/한국|Korea|首尔|서울|호텔|Hotel|酒店/i.test(s) && !/广州|廣州|Guangzhou|Canton|Tower|广州塔|中國|中国/i.test(s))
                return false;
              return /广州|廣州|Guangzhou|Canton|Tower|广州塔|廣州塔|海心沙|Haixinsha|中国|中國/i.test(s);
            }) || null;
          if (hit) {
            hit.click();
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '마카오 팀랩 슈퍼네이처 1일 입장권' }],
...
async function options(page) {
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('마카오 팀랩 슈퍼네이처 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
