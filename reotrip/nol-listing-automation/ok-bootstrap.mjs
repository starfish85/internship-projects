/**
 * 大阪市区酒店-京都市区酒店 — 创建或恢复草稿
 * Excel (8) 日本接送 R49–52 · 7/10 去返 627/1020 · 无假日
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '오사카 시내 호텔 ↔ 교토 시내 호텔 단독 차량 편도 이동 서비스';
const INTERNAL = '大阪市区酒店-京都市区酒店';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 京都站 草稿');
killPeerCdpScripts('ok-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /오사카 시내 호텔.*교토 시내|大阪市区酒店-京都市区酒店/i.test(body)) {
  console.log('【将要】恢复草稿 京都站');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /大阪市区酒店-京都市区酒店|오사카 시내 호텔 ↔ 교토 시내 호텔/ })
    .filter({ hasNotText: /京都站|교토역|Kyoto Station/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.ok-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.ok-draft-id', import.meta.url), m[1]);
process.exit(0);
