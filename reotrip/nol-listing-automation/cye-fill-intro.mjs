...
const DRAFT = process.env.CYE_DRAFT || readFileSync(join(__dir, '.cye-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/cts-yotei';
const IMAGES = ['cye-1.jpg', 'cye-2.jpg', 'cye-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '신치토세 공항(CTS)';
const B = '요테이산';
...
const INTRO = `이 서비스는 ${A}(New Chitose Airport)와 ${B}(Mt. Yotei) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명·도착/출발 시간·터미널, 요테이산 호텔·숙소·픽업 장소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

신치토세 공항 출도착 후 요테이산로 바로 이동하시거나, 관광 후 공항으로 이동하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 항공편·터미널 정보와 요테이산 호텔·픽업 장소, 이용 시간을 정확히 입력해 주세요.
...
3.예약 시 항공편명, 도착 또는 출발 시간, 터미널, 요테이산 주소, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('cye-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /신치토세|CTS|요테이산|Chitose/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /신치토세|CTS|요테이산|Chitose|Hokkaido/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
