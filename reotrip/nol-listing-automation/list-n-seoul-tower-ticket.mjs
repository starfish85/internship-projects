/**
 * N Seoul Tower / n tower Observation Deck
 * CSV 韩国门票产品 R3: 一日票 · 只卖成人 · 外籍特惠（不支持韩国国籍）
 * 【售价来源】售价hkd=139（表显示；加价逻辑 112 未用；未自算）skill 0h
 * 单选项 1일 입장권；0n 双门禁 代表+按数量
 * 串行：本脚本只上 n tower，禁并行其他韩国门票
 * Never 提交审核
 *
 * usage: node list-n-seoul-tower-ticket.mjs [bootstrap|attrs|intro|regs|options]
 */
...
const DRAFT_FILE = join(__dir, '.n-seoul-tower-draft-id');
const PHASE = process.argv[2] || 'bootstrap';
// 【售价来源】售价hkd=252（CSV 韩国门票 R3 表显示；加价逻辑=112 未用；未自算）
const PRICE = (process.env.N_SEOUL_TOWER_PRICE || process.argv[3] || '139').trim();
const PRODUCT_KO = 'N 서울타워 전망대 1일 입장권 (외국인 특혜)';
const INTERNAL = 'n tower / N Seoul Tower 1日票 外籍特惠';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/n-seoul-tower';
const IMAGES = ['nt-1.jpg', 'nt-2.jpg', 'nt-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '남산 N 서울타워에서 서울 전경을 감상하세요!';
const HIGHLIGHT = [
  'N 서울타워 전망대 1일 입장권 · 성인 전용 · 외국인 특혜 요금',
  '남산 랜드마크 전망 · 이메일 전자티켓/이용 안내',
  '한국 국적자 구매 불가 · 예약 시 전화·이메일·여권 정보 필수',
].join('\n');
const INTRO = `N 서울타워(N Seoul Tower) 전망대 1일 입장권입니다.
남산 정상에 위치한 서울의 대표 전망 타워에서 도심 전경을 조망할 수 있습니다.

포함 사항:
- N 서울타워 전망대 1일 입장권 1매 (성인)

중요 안내:
- 본 상품은 외국인 특혜 요금입니다. 한국 국적 보유자는 구매·이용이 불가합니다.
- 입장 시 여권 등 국적 확인이 요청될 수 있으니 반드시 지참해 주세요.

이용 안내:
- 티켓/이용 안내는 이용일 기준 보통 7일 이내에 예약 시 입력한 이메일로 발송됩니다. 이메일 주소가 정확하고 첨부파일을 수신할 수 있는지 확인해 주세요.
- 예약 시 모든 이용자의 영문 성명·여권번호와 연락 가능한 전화·이메일을 정확히 입력해 주세요.
- 본 상품은 성인 전용입니다.

서울 야경·시티뷰를 즐기고 싶은 외국인 이용객께 추천합니다.`;
const MUST_KNOW = `1. 본 상품은 외국인 특혜 요금이며, 한국 국적 보유자는 구매 및 이용이 불가합니다. 입장 시 여권 등 국적 확인 서류를 지참해 주세요.

2. 티켓/이용 안내는 이용일 기준 보통 7일 이내에 예약 이메일로 발송됩니다. 이메일이 실제 수신 가능하고 첨부파일을 받을 수 있는지 확인해 주세요.

3. 예약 시 모든 이용자의 영문 성명과 여권번호, 연락 가능한 전화번호·이메일을 정확히 입력해 주세요.
...
2. 안내에 따라 지정 입구에 맞춰 입장합니다. 외국인 특혜 요금이므로 여권 등 국적 확인 서류를 반드시 지참하세요.

3. 문의: agency@reotrip.com / +852 3428 81 82`;
const OPT = {
  name: '1일 입장권',
  desc: `N 서울타워 전망대 1일 입장권
성인 전용 · 외국인 특혜 요금
한국 국적자 구매 불가 · 여권 지참
이메일 전자티켓/이용 안내 기준 입장
예약 시 전화·이메일·영문성명·여권번호 필수`,
  pt: '성인',
  ptd: '성인 전용 · 만 19세 이상',
  price: PRICE,
};
...
function readDraft() {
  if (process.env.N_SEOUL_TOWER_DRAFT) return process.env.N_SEOUL_TOWER_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=112 未用作售价；未自算）');
  ...
  // resume 必须 N 서울타워 / n tower；禁 롯데월드타워
  if (/N 서울타워|N Seoul Tower|n tower|남산타워|Namsan Tower|N타워/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /N 서울타워|N Seoul Tower|n tower|남산타워|Namsan/i })
      .filter({ hasNotText: /롯데월드타워|Lotte World Tower/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /N 서울|N Seoul|남산|Namsan|n tower|N타워|N Tower/.test(t)
...
async function attrs(page) {
  ...
  if (!st.hasTheme) {
    ...
    if ((await themeBtn.count()) > 0) {
      ...
      let picked = false;
      // 真选中：label[for=categoryIds_*] checked — 乐园优先 主题公园
      picked = await page.evaluate(() => {
        const dlg = document.querySelector('[role=dialog]');
        if (!dlg) return false;
        const want = ['观景台', '觀景台', '景点', '景點', '观光', '觀光', '体验'];
        for (const name of want) {
          const lab = Array.from(dlg.querySelectorAll('label')).find((l) => {
          ...
          if (inp && inp.checked) return name;
          if (inp && !inp.checked) lab.click();
          else if (!inp) lab.click();
          if (inp?.checked) return name;
          return name;
        }
        return false;
      });
  ...
  // POI — 首尔 乐天塔/롯데월드타워（禁 纯乐园·酒店·中国）
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI N 서울타워 / N Seoul Tower');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['N Seoul Tower', 'N 서울타워', '남산타워', 'Namsan Tower', 'n tower'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const score = (s) => {
            if (/Hotel|호텔|酒店|숙박|住宿/i.test(s) && !/타워|Tower|전망/i.test(s)) return -50;
            if (/广州|廣州|北京|上海|Macau|澳门|롯데월드타워|Lotte World Tower/i.test(s)) return -50;
            if (/N 서울타워|N Seoul Tower|남산타워|Namsan Tower|N타워/i.test(s) && /서울|Seoul|남산|Namsan|용산|Yongsan|韩国|Korea/i.test(s))
              return 100;
            if (/N 서울타워|N Seoul Tower|남산타워|Namsan Tower/i.test(s)) return 80;
            return 0;
          };
          let best = null;
          let bs = 0;
          for (const e of items) {
            const sc = score(e.innerText || '');
            if (sc > bs) {
              bs = sc;
              best = e;
            }
          ...
          if (best && bs >= 80) {
            best.click();
            return (best.innerText || '').slice(0, 120);
          }
          return null;
        });
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: 'N 서울타워 전망대 1일 입장권' }],
...
async function options(page) {
  if (!PRICE) {
    ...
    console.log('请设置: TEAMLAB_MACAO_PRICE=<表显示> node list-canton-tower-450-ticket.mjs options');
    process.exit(2);
  }
  const id = readDraft();
  if (!id) throw new Error('no draft');
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('N 서울타워 전망대 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
