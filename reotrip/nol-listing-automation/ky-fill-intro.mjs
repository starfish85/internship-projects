...
const DRAFT = process.env.KY_DRAFT || readFileSync(join(__dir, '.ky-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/kyoto-station';
const IMAGES = ['ky-1.jpg', 'ky-2.jpg', 'ky-3.jpg']
...
const HIGHLIGHT = [
  ...
  '교토역 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${CITY}과 ${DEST}(京都駅 / Kyoto Station) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 호텔명·주소, 교토역 출구/미팅 장소, 픽업/샌딩 장소, 이용 시간, 열차 정보, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 소규모 그룹 등 교토역를 이용하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 4개까지 적재 가능. 차량 공간을 고려해 적합한 차형을 선택해 주세요.

2.미팅 장소: 호텔 주소와 교토역 픽업/샌딩 장소·이용 시간을 정확히 입력해 주세요.
...
3.예약 시 호텔명·주소, 역 출구/미팅 장소, 픽업/샌딩 시간, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('ky-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /교토역|Kyoto Station|スカイ/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: (document.querySelector('#description, textarea[name=description]')?.value || '').includes('Kyoto Station') || (document.querySelector('#description, textarea[name=description]')?.value || '').includes('교토역'),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Tokyo Kyoto Station');
