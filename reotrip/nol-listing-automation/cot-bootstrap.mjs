/**
 * Excel(11) R208–211 · 7/10 售价3431/3769 · CTS-小樽 航班 · 车图
 * Excel(11) R208–211 · 7/10 售价3431/3769 · CTS-小樽 航班 · 车图
 * Excel(11) R208–211 · 7/10 售价3431/3769 · CTS-小樽 航班 · 车图
 */
...
const PRODUCT_KO = '신치토세 공항(CTS) ↔ 오타루 단독 차량 편도 이동 서비스';
const INTERNAL = '新千岁机场(CTS)-小樽';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 新千岁机场(CTS)-小樽 草稿');
killPeerCdpScripts('cot-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /신치토세.*오타루|新千岁机场\(CTS\)-小樽|小樽|Otaru/i.test(body)) {
  console.log('【将要】恢复草稿 新千岁机场(CTS)-小樽');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /新千岁机场\(CTS\)-小樽|新千岁.*迪士尼|신치토세 공항\(CTS\) ↔ 도쿄 교토/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.cot-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.cot-draft-id', import.meta.url), m[1]);
process.exit(0);
