 ...
 * 【售价来源】售价hkd=46.54（表显示；加价逻辑空 未用；未自算）skill 0h
 * Family A: adult only, 2 session options, email ticket, passport+name
 * Never 提交审核
 *
 * usage: node list-badaling-great-wall-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 */
...
const DRAFT_FILE = join(__dir, '.badaling-great-wall-draft-id');
const PHASE = process.argv[2] || 'all';
// 硬规则 0h：只贴表显示售价
// 【售价来源】售价hkd=46.54（CSV R36–R37 表显示；加价逻辑空 未用；未自算）
const PRICE = (process.env.BADALING_PRICE || process.argv[3] || '46.54').trim();
const PRODUCT_KO = '베이징 팔달령 만리장성 입장권';
const INTERNAL = 'Beijing Badaling Great Wall Admission Ticket';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/badaling-great-wall';
const IMAGES = ['badaling-1.jpg', 'badaling-2.jpg', 'badaling-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '베이징 팔달령 만리장성을 오전·오후 세션으로 즐겨보세요!';
const HIGHLIGHT = [
  '팔달령 만리장성 입장권 · 오전/오후 세션 선택',
  '성인 전용 · 이메일 전자티켓/이용 안내',
  '예약 시 영문 성명·여권번호 필수',
].join('\n');
const INTRO = `베이징 팔달령(Badaling) 만리장성 입장권입니다.
오전 세션과 오후 세션 중 선택하여 만리장성의 대표적인 구간을 관람할 수 있습니다.

포함 사항:
- 선택한 세션(오전 또는 오후) 팔달령 만리장성 입장권 1매 (성인)
...
// Family A: 双选项 上午/下午 · 价=表显示 46.54
const OPTIONS = [
  {
    key: 'am',
    name: '오전 세션 입장권',
    desc: `팔달령 만리장성 오전 세션 입장권
성인 전용
...
function readDraft() {
  if (process.env.BADALING_DRAFT) return process.env.BADALING_DRAFT.trim();
...
async function bootstrap(page) {
  ...
  if (/Badaling|八达岭|팔달령|팔달/i.test(body) && !/Mutianyu|慕田峪|뮤톈위|뮤톈/i.test(body.split('\n').find((l) => /Badaling|八达岭|팔달/i.test(l)) || '')) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /Badaling|八达岭|팔달령/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /北京|Beijing|Badaling|八达岭|뮤톈|长城|長城|만리장성|延庆|延慶|Yanqing/.test(t)
...
async function attrs(page) {
  ...
  // POI — 八达岭 / Badaling（优先旅游地；穷尽后可就近住宿）
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 八达岭 / Badaling Great Wall');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['八达岭', 'Badaling', '八达岭长城', 'Badaling Great Wall', '北京 八达岭'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const hit =
            items.find((e) => {
              const s = e.innerText || '';
              if (/한국|Korea|首尔|서울/i.test(s) && !/北京|Beijing|Badaling|八达岭|中国|中國/i.test(s))
                return false;
              // 优先非酒店
              if (/호텔|Hotel|酒店|住宿/i.test(s) && !/Badaling|八达岭/i.test(s)) return false;
              return /八达岭|Badaling|长城|長城|Great Wall|北京|Beijing|延庆|延慶|Yanqing|中国|中國/i.test(s);
            }) || null;
          if (hit) {
            hit.click();
      ...
      // 穷尽后住宿兜底：同城+八达岭关键词
      if (!clicked) {
        console.log('【POI 兜底尝试】就近住宿 八达岭/延庆');
        for (const q of ['Badaling Hotel', '八达岭 酒店', '延庆 长城']) {
          await search.fill('');
          await search.fill(q);
          await search.press('Enter');
          await sleep(2000);
          clicked = await page.evaluate(() => {
            ...
            const hit = items.find((e) => {
              ...
              return /北京|Beijing|Badaling|八达岭|延庆|延慶|Yanqing|长城|長城/i.test(s);
            });
            if (hit) {
              hit.click();
...
async function regs(page) {
  ...
  try {
    ...
    if ((await writeInc.count()) > 0) {
      ...
      if ((await desc.count()) > 0) await desc.fill('팔달령 만리장성 입장권').catch(() => {});
      await page.evaluate(() => {
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '팔달령 만리장성 입장권' }],
