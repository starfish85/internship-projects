/**
 * 成田机场(NRT)-羽田机场(HND) 介绍 + 用户3图 → 保存然后
 * 机场产品：航班预约
 */
import fs from 'fs';
...
const DRAFT = process.env.NHH_DRAFT || readFileSync(join(__dir, '.nhh-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/narita-haneda';
const IMAGES = ['nhh-1.jpg', 'nhh-2.jpg', 'nhh-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '나리타 공항(NRT)';
const B = '하네다 공항(HND)';
const HEADLINE = `${A} ↔ ${B} 편도 전용 차량으로 여유로운 이동을 즐기세요!`;
const HIGHLIGHT = [
  `${A}과 ${B}를 편안하게 연결하는 단독 차량 이동`,
  '항공편 일정에 맞춘 프라이빗 공항 간 이동',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${A}(Narita Airport)와 ${B}(Haneda Airport) 사이의 편도 전용 차량 이동 서비스입니다.
편안하고 프라이빗한 차량과 숙련된 기사님이 함께하여, 대중교통 환승이나 택시 이용 없이 두 공항 사이를 빠르고 쾌적하게 이동하실 수 있습니다.

포함 사항:
- ${A} ↔ ${B} 편도 전용 차량 서비스 1회
- 차량 및 기사 요금, 주차비 포함

예약 시 양쪽 항공편명·도착/출발 시간·터미널, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

나리타·하네다 환승, 출국/입국 일정이 다른 경우 등 두 공항 간 이동이 필요하신 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 양쪽 항공편·터미널 정보와 이용 시간을 정확히 입력해 주세요.
...
3.예약 시 항공편명, 도착 또는 출발 시간, 터미널, 하네다 공항(HND) 주소, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('nhh-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /나리타|하네다|NRT|HND|Narita|Haneda/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /나리타|하네다|NRT|HND|Narita|Haneda/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing NRT-HND');
