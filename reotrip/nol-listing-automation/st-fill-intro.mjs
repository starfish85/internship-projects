...
const DRAFT = process.env.ST_DRAFT || readFileSync(join(__dir, '.st-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/tokyo-skytree';
const IMAGES = ['st-1.jpg', 'st-2.jpg', 'st-3.jpg']
...
const HIGHLIGHT = [
  ...
  '스카이트리 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${CITY}과 ${DEST}(東京スカイツリー / Tokyo Skytree) 사이의 편도 전용 차량 이동 서비스입니다.
...
가족, 출장, 소규모 그룹 등 도쿄 스카이트리를 이용하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 4개까지 적재 가능. 차량 공간을 고려해 적합한 차형을 선택해 주세요.

2.미팅 장소: 호텔 주소와 스카이트리 픽업/샌딩 장소·이용 시간을 정확히 입력해 주세요.
...
const textOk = await page.evaluate(() => ({
  hl: /스카이트리|Skytree|スカイ/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: (document.querySelector('#description, textarea[name=description]')?.value || '').includes('Skytree') || (document.querySelector('#description, textarea[name=description]')?.value || '').includes('스카이트리'),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Tokyo Skytree');
