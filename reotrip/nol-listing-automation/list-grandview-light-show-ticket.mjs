/**
 * 广州正佳广场 幻彩光影秀 Grandview Plaza Fantasy Light Show
 * 用户截图表：1日票 · 只卖成人 · 无需护照号（入园 copy）
 * 【售价来源】售价hkd=223（截图表显示；加价逻辑 156 未用；未自算）skill 0h
 * 单选项 1일 입장권；0n 双门禁（表无需护照≠跳过按数量）
 * Never 提交审核
 *
 * usage: node list-grandview-light-show-ticket.mjs [bootstrap|attrs|intro|regs|options]
 */
...
const DRAFT_FILE = join(__dir, '.grandview-light-show-draft-id');
const PHASE = process.argv[2] || 'bootstrap';
// 【售价来源】售价hkd=194（用户截图表显示；加价逻辑=156 未用；未自算）
const PRICE = (process.env.GRANDVIEW_LIGHT_PRICE || process.argv[3] || '223').trim();
const PRODUCT_KO = '광저우 정자광장 환채광영쇼 1일 입장권';
const INTERNAL = '幻彩光影秀 / Grandview Plaza Fantasy Light Show';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/grandview-light-show';
// 0m：N=用户提供张数；现目录 3 张则传 3
const IMAGES = ['gv-1.jpg', 'gv-2.jpg', 'gv-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '광저우 정자광장 환채광영쇼의 화려한 야경을 만나보세요!';
const HIGHLIGHT = [
  '정자광장 환채광영쇼 1일 입장권 · 성인 전용',
  '광저우 랜드마크 미디어 파사드 쇼 · 이메일 전자티켓/이용 안내',
  '예약 시 전화·이메일 필수 (여권번호 불필요)',
].join('\n');
const INTRO = `광저우 정자광장(Grandview Plaza) 환채광영쇼(幻彩光影秀) 1일 입장권입니다.
쇼핑몰 외벽 미디어 파사드를 활용한 야간 라이트·미디어 쇼를 관람할 수 있습니다.
...
- 예약 시 연락 가능한 전화번호와 이메일을 정확히 입력해 주세요. (본 상품은 여권번호 입력이 필요하지 않습니다.)
- 본 상품은 성인 전용입니다.

광저우 야경·미디어 아트를 즐기고 싶은 분들께 추천합니다.`;
const MUST_KNOW = `1. 티켓/이용 안내는 이용일 기준 보통 7일 이내에 예약 이메일로 발송됩니다. 이메일이 실제 수신 가능하고 첨부파일을 받을 수 있는지 확인해 주세요.

2. 예약 시 연락 가능한 전화번호와 이메일을 정확히 입력해 주세요. 본 상품은 여권번호 입력이 필요하지 않습니다.

3. 본 상품은 성인 전용이며 아동/노인 요금은 판매하지 않습니다.

4. 입장·이용 규정은 이메일 전자티켓/이용 안내 및 현장 공지를 따릅니다. 쇼 상영 시간·좌석/관람 위치는 현장 안내에 따라 주세요.`;
const HOW_TO = `1. 예약 확정 후, 이용일 기준 보통 7일 이내에 이메일로 전자티켓 또는 이용 안내를 수령합니다.

2. 안내에 따라 지정 관람 구역·시간에 맞춰 입장합니다.

3. 문의: agency@reotrip.com / +852 3428 81 82`;
const OPT = {
  name: '1일 입장권',
  desc: `광저우 정자광장 환채광영쇼 1일 입장권
성인 전용
...
function readDraft() {
  if (process.env.GRANDVIEW_LIGHT_DRAFT) return process.env.GRANDVIEW_LIGHT_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=156 未用作售价；未自算）');
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /正佳|Grandview|广州|廣州|Guangzhou|정자|Tianhe|天河/.test(t)
...
async function attrs(page) {
  ...
  if (!st.hasTheme) {
    ...
    if ((await themeBtn.count()) > 0) {
      ...
      picked = await page.evaluate(() => {
        ...
        if (!dlg) return false;
        const want = ['景点', '景點', '体验', '體驗', '观光', '觀光', '旅游地'];
  ...
  // POI — 广州 正佳广场 Grandview Plaza
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 正佳广场 / Grandview Plaza Guangzhou');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['Grandview Plaza', '正佳广场', 'Guangzhou Grandview', '正佳廣場', '정자광장'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const score = (s) => {
            if (/한국|Korea|首尔|서울|Hotel|호텔|酒店|숙박/i.test(s) && !/正佳|Grandview/i.test(s)) return -50;
            if (/北京|上海|Macau|澳门|長隆|长隆|Chimelong|广州塔|Canton Tower/i.test(s)) return -40;
            let sc = 0;
            if (/正佳|Grandview/i.test(s)) sc += 90;
            if (/广州|廣州|Guangzhou|天河|Tianhe/i.test(s)) sc += 20;
            if (/旅游地|旅遊地/i.test(s)) sc += 15;
            if (/住宿|Hotel|酒店/i.test(s)) sc -= 35;
            return sc;
          };
          let best = null;
          let bs = 0;
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '광저우 정자광장 환채광영쇼 1일 입장권' }],
...
async function options(page) {
  if (!PRICE) {
    ...
    console.log('请设置: GRANDVIEW_LIGHT_PRICE=<表显示> node list-canton-tower-450-ticket.mjs options');
    process.exit(2);
  }
  const id = readDraft();
  if (!id) throw new Error('no draft');
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('광저우 정자광장 환채광영쇼 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
