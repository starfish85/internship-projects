 ...
 * CSV 韩国门票产品 R4: 一日票 · 只卖成人 · 外籍特惠（不支持韩国国籍）
 * 【售价来源】售价hkd=277（表显示；加价逻辑 222 未用；未自算）skill 0h
 * 单选项 1일 입장권；0n 双门禁 代表+按数量
 * 串行：本脚本只上 everland，禁并行其他韩国门票
 * Never 提交审核
 *
 * usage: node list-everland-ticket.mjs [bootstrap|attrs|intro|regs|options]
 */
...
const DRAFT_FILE = join(__dir, '.everland-draft-id');
const PHASE = process.argv[2] || 'bootstrap';
// 【售价来源】售价hkd=252（CSV 韩国门票 R4 表显示；加价逻辑=222 未用；未自算）
const PRICE = (process.env.EVERLAND_PRICE || process.argv[3] || '277').trim();
const PRODUCT_KO = '에버랜드 1일 입장권';
const INTERNAL = 'everland / Everland 1日票';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/everland';
const IMAGES = ['ev-1.jpg', 'ev-2.jpg', 'ev-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '에버랜드에서 하루 종일 어드벤처를 즐기세요!';
const HIGHLIGHT = [
  '에버랜드 1일 입장권 · 성인 전용',
  '테마파크 종일 이용 · 이메일 전자티켓/이용 안내',
  '예약 시 전화·이메일·영문성명·여권번호 필수',
].join('\n');
const INTRO = `에버랜드(Everland) 1일 입장권입니다.
경기도 용인의 대표 테마파크에서 다양한 어트랙션과 쇼를 하루 종일 즐길 수 있습니다.
...
function readDraft() {
  if (process.env.EVERLAND_DRAFT) return process.env.EVERLAND_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=222 未用作售价；未自算）');
  ...
  // resume everland only
  if (/에버랜드|Everland|everland/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /에버랜드|Everland|everland/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /에버랜드|Everland|everland|용인|Yongin/.test(t)
...
async function attrs(page) {
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 에버랜드 / Everland');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['Everland', '에버랜드', 'Everland Yongin', '용인 에버랜드'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const score = (s) => {
            if (/Hotel|호텔|酒店|숙박|住宿/i.test(s) && !/Everland|에버랜드|테마/i.test(s)) return -50;
            if (/广州|廣州|北京|上海|Macau|N 서울|남산타워|롯데/i.test(s)) return -50;
            if (/에버랜드|Everland/i.test(s) && /용인|Yongin|경기|Gyeonggi|韩国|Korea|서울|Seoul/i.test(s)) return 100;
            if (/에버랜드|Everland/i.test(s)) return 85;
            return 0;
          };
          let best = null;
          let bs = 0;
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '에버랜드 1일 입장권' }],
...
async function options(page) {
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('에버랜드 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
