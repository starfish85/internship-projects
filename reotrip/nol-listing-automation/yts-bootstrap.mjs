/**
 * 横滨邮轮港-东京站 — 创建或恢复草稿
 * Excel(11) R160–163 · 7/10 成本→售价1255/1501 · 港口无航班 日本接送 R86–89 · 7/10 成本737.25/972.55 → 售价1053/1389 · 无假日
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '요코하마 크루즈항 ↔ 도쿄역 단독 차량 편도 이동 서비스';
const INTERNAL = '横滨邮轮港-东京站';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 横滨邮轮港-东京站 草稿');
killPeerCdpScripts('yts-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /요코하마 크루즈항.*도쿄역|横滨邮轮港-东京站/i.test(body)) {
  console.log('【将要】恢复草稿 横滨邮轮港-东京站');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /横滨邮轮港-东京站|요코하마 크루즈항 ↔ 도쿄역/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.yts-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.yts-draft-id', import.meta.url), m[1]);
process.exit(0);
