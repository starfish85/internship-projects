/**
 * Macau Tower Observation Deck Admission Ticket
 * CSV 国内景区门票 (3) R45: 1日票 · 只卖成人
 * 【售价来源】售价hkd=194（表显示；加价逻辑 135.7 未用；未自算）skill 0h
 * 单选项 1일 입장권
 * 0n 双门禁：代表 电话+邮箱 + 按数量 英文姓·名·护照（表「无需护照」=入园 copy，≠跳过按数量 UI）
 * Never 提交审核
 *
 * usage: node list-macau-tower-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 */
...
const DRAFT_FILE = join(__dir, '.macau-tower-draft-id');
const PHASE = process.argv[2] || 'all';
// 【售价来源】售价hkd=194（CSV (3) R45 表显示；加价逻辑=135.7 未用；未自算）
const PRICE = (process.env.MACAU_TOWER_PRICE || process.argv[3] || '194').trim();
const PRODUCT_KO = '마카오 타워 전망대 1일 입장권';
const INTERNAL = 'Macau Tower Observation Deck Admission Ticket';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/macau-tower';
// 0m：N=用户提供张数；现目录 3 张则传 3
const IMAGES = ['tower-1.jpg', 'tower-2.jpg', 'tower-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '마카오 타워 전망대에서 마카오 시티 스카이라인을 감상하세요!';
const HIGHLIGHT = [
  '마카오 타워 전망대 1일 입장권 · 성인 전용',
  '마카오 시내·해안 전망 · 이메일 전자티켓/이용 안내',
  '예약 시 연락 가능한 전화·이메일 필수 (여권번호 불필요)',
].join('\n');
const INTRO = `마카오 타워(Macau Tower) 전망대 1일 입장권입니다.
높은 전망대에서 마카오 도심과 해안 일대를 조망할 수 있습니다.

포함 사항:
- 마카오 타워 전망대 1일 입장권 1매 (성인)
...
마카오의 랜드마크 전망을 즐기고 싶은 분들께 추천합니다.`;
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=135.7 未用作售价；未自算）');
  ...
  if (/마카오 타워|Macau Tower|澳门塔|澳門塔|전망대 1일/i.test(body) && !/TeamLab|팀랩|teamlab/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /마카오 타워|Macau Tower|澳门塔|澳門塔/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /澳门|澳門|Macau|Macao|마카오|塔|Tower|타워/.test(t)
...
async function attrs(page) {
  ...
  if (!st.hasTheme) {
    ...
    if ((await themeBtn.count()) > 0) {
      ...
      picked = await page.evaluate(() => {
        ...
        if (!dlg) return false;
        const want = ['观景台', '觀景台', '观光', '觀光', '景点', '景點'];
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 澳门塔 / Macau Tower');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['Macau Tower', '澳门塔', '澳門塔', '마카오 타워', 'Macao Tower'];
      let clicked = null;
