...
const DRAFT = process.env.CTY_DRAFT || readFileSync(join(__dir, '.cty-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/cts-toya';
const IMAGES = ['cty-1.jpg', 'cty-2.jpg', 'cty-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '신치토세 공항(CTS)';
const B = '도야코';
...
const INTRO = `이 서비스는 ${A}(New Chitose Airport)와 ${B}(도야코 / Lake Toya, 北海道) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명·도착/출발 시간·터미널, 도야코 호텔/픽업·샌딩 장소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

신치토세 공항(CTS) 출도착 후 도야코(洞爺湖)로 이동하시거나, 도야코에서 공항으로 이동하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 항공편·터미널 정보와 도야코 호텔/픽업 장소, 이용 시간을 정확히 입력해 주세요.
...
3.예약 시 항공편명, 도착 또는 출발 시간, 터미널, 도야코 주소, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('cty-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /신치토세|도야코|CTS|Chitose|Toya/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /신치토세|도야코|CTS|Chitose|Toya|洞爺|洞爷/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing CTS-Toya');
