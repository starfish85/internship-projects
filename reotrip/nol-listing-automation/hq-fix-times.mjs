/**
 * 虹桥站 时段 — 仅调用 lib/set-times-china.mjs
 * usage: node hq-fix-times.mjs <0|1|…>   # Live 按卡分批；禁止默认 all 与 holidays 同链
 */
...
if (!arg) {
  console.error('用法: node hq-fix-times.mjs <optionIndex|all>  — Live 请按卡: 0 然后 1；勿与 holidays 同链');
  process.exit(2);
}
if (arg === 'all') console.warn('【警告】all 会连跑多卡；Live 推荐分批 0 / 1');


const { page } = await connectNolPage({ selfHint: 'hq-fix-times', killPeers: true });
...
for (const i of indices) {
  try {
    ...
    console.log('【提示】只重跑本卡: node hq-fix-times.mjs', i, '；勿 silent 换第二套 setTimes');
    process.exit(2);
  }
}
