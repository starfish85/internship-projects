/**
 * 大阪港-关西机场(KIX) — 创建或恢复草稿
 * Excel(11) R176–179 · 7/10 成本→售价1109/1490 · 港+机场 航班+船
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '오사카항 ↔ 간사이 국제공항(KIX) 단독 차량 편도 이동 서비스';
const INTERNAL = '大阪港-关西机场(KIX)';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 大阪港-关西机场(KIX) 草稿');
killPeerCdpScripts('opk-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /오사카항.*간사이|大阪港-关西|大阪港.*KIX/i.test(body)) {
  console.log('【将要】恢复草稿 大阪港-关西机场(KIX)');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /大阪港-关西机场|大阪港.*KIX|오사카항 ↔ 간사이/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.opk-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.opk-draft-id', import.meta.url), m[1]);
process.exit(0);
