/**
 * Simatai Great Wall Admission Ticket + round-trip cable car
 * CSV 国内景区门票 (2) R42–R43: 上午场/下午场 · 只卖成人
 * 【售价来源】售价hkd=319（表显示；加价逻辑 223.36 未用；未自算）skill 0h
 * Family A: adult only, 2 session combo options, email ticket, passport+name
 * Never 提交审核
 *
 * usage: node list-simatai-combo-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 */
...
const DRAFT_FILE = join(__dir, '.simatai-combo-draft-id');
const PHASE = process.argv[2] || 'all';
// 硬规则 0h：只贴表显示售价
// 【售价来源】售价hkd=52.35（CSV R34–R35 表显示；加价逻辑空 未用；未自算）
const PRICE = (process.env.SIMATAI_COMBO_PRICE || process.argv[3] || '319').trim();
const PRODUCT_KO = '베이징 스마타이 만리장성 입장+왕복 케이블카';
const INTERNAL = 'Simatai Great Wall Admission Ticket + round-trip cable car';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/simatai-combo';
const IMAGES = ['combo-1.jpg', 'combo-2.jpg', 'combo-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '베이징 스마타이 만리장성 입장+왕복 케이블카 콤보!';
const HIGHLIGHT = [
  '스마타이 만리장성 입장권 + 왕복 케이블카',
  '오전/오후 세션 선택 · 성인 전용',
  '이메일 전자티켓/이용 안내 · 예약 시 영문 성명·여권번호 필수',
].join('\n');
const INTRO = `베이징 스마타이(Simatai) 만리장성 입장권과 왕복 케이블카가 포함된 콤보 상품입니다.
오전 세션과 오후 세션 중 선택하여 스마타이 구간을 관람할 수 있습니다.

포함 사항:
- 선택한 세션(오전 또는 오후) 스마타이 만리장성 입장권 1매 (성인)
- 왕복 케이블카 이용권 1매 (성인)
...
- 본 상품은 성인 전용입니다.
- 입장 시간대·입구·케이블카 탑승 위치·이용 규정은 이메일 전자티켓/이용 안내를 따릅니다.

베이징 근교에서 스마타이 만리장성과 케이블카를 함께 이용하고 싶은 분들께 추천합니다.`;
...
5. 입장 시간·입구·왕복 케이블카 탑승·현장 규정은 이메일 전자티켓/이용 안내 및 현장 공지를 따릅니다.`;

const HOW_TO = `1. 예약 확정 후, 이용일 기준 보통 7일 이내에 이메일로 전자티켓 또는 이용 안내를 수령합니다.

2. 안내에 따라 지정 입구·세션 시간대에 맞춰 입장합니다.

3. 왕복 케이블카는 안내에 명시된 탑승 위치에서 이용합니다.

4. 입장 시 여권 등 신분 확인이 필요할 수 있으니 여권을 지참해 주세요.

5. 문의: agency@reotrip.com / +852 3428 81 82`;
...
const FAQ = [
  ...
  {
    q: '콤보에 무엇이 포함되나요?',
    a: '스마타이 만리장성 입장권과 왕복 케이블카가 포함됩니다. 셔틀버스 등 기타 항목은 포함되지 않을 수 있으니 이용 안내를 확인해 주세요.',
  },
  {
...
// Family A: 双选项 上午/下午 콤보 · 价=表显示 319
const OPTIONS = [
  {
    key: 'am',
    name: '오전 세션 콤보',
    desc: `스마타이 만리장성 오전 세션 콤보
입장권 + 왕복 케이블카
성인 전용
...
function readDraft() {
  if (process.env.SIMATAI_COMBO_DRAFT) return process.env.SIMATAI_COMBO_DRAFT.trim();
...
async function bootstrap(page) {
  ...
  // 仅 resume 司马台套餐：必须含 스마타이/Simatai；禁 뮤톈위/纯입장권/慕田峪
  if (/스마타이.*케이블|스마타이.*콤보|Simatai.*(cable|combo)/i.test(body) && !/뮤톈위|Mutianyu|慕田峪/i.test(body.split('\n').find(l=>/케이블|콤보|cable/i.test(l))||'')) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /스마타이/i })
      .filter({ hasText: /케이블|콤보|cable/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /北京|Beijing|Simatai|司马台|司馬台|스마타이|사마대|密云|密雲|Miyun|长城|長城|만리장성|古北|Gubei/.test(t)
...
async function attrs(page) {
  ...
  // POI — 司马台 / Simatai（优先旅游地；穷尽后可就近住宿）
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 司马台 / Simatai Great Wall');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['司马台', 'Simatai', '司马台长城', 'Simatai Great Wall', '北京 司马台', '사마대'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const hit =
            items.find((e) => {
              const s = e.innerText || '';
              if (/한국|Korea|首尔|서울/i.test(s) && !/北京|Beijing|Simatai|司马台|司馬台|中国|中國/i.test(s))
                return false;
              // 优先非酒店
              if (/호텔|Hotel|酒店|住宿/i.test(s) && !/Simatai|司马台|司馬台|사마대/i.test(s)) return false;
              return /司马台|司馬台|Simatai|사마대|스마타이|密云|密雲|Miyun|古北|Gubei|长城|長城|Great Wall|北京|Beijing|中国|中國/i.test(s);
            }) || null;
          if (hit) {
            hit.click();
      ...
      // 穷尽后住宿兜底：同城+慕田峪关键词
      if (!clicked) {
        console.log('【POI 兜底尝试】就近住宿 司马台/密云/古北');
        for (const q of ['Simatai Hotel', '司马台 酒店', '古北水镇', '密云 长城']) {
          await search.fill('');
          await search.fill(q);
          await search.press('Enter');
          await sleep(2000);
          clicked = await page.evaluate(() => {
            ...
            const hit = items.find((e) => {
              ...
              return /北京|Beijing|Simatai|司马台|司馬台|密云|密雲|Miyun|古北|Gubei|长城|長城/i.test(s);
            });
            if (hit) {
              hit.click();
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '스마타이 입장+왕복 케이블카' }],
...
async function options(page) {
  ...
  const hasAm = /오전 세션 콤보/.test(body0);
  const hasPm = /오후 세션 콤보/.test(body0);
  ...
  const final = await page.evaluate((price) => {
    ...
    return {
      ...
      hasAm: /오전 세션 콤보/.test(t),
      hasPm: /오후 세션 콤보/.test(t),
      sale: /销售中|銷售中/.test(t),
