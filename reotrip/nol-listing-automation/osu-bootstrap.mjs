/**
 * 大阪市区酒店-铃鹿市区 — 创建或恢复草稿
 * Excel (9) 日本接送 R74–77 · 7/10 成本2274.51/2666.67 → 售价3249/3810 · 无假日
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '오사카 시내 호텔 ↔ 스즈카 시내 단독 차량 편도 이동 서비스';
const INTERNAL = '大阪市区酒店-铃鹿市区';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 大阪市区酒店-铃鹿市区 草稿');
killPeerCdpScripts('osu-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /오사카 시내 호텔.*스즈카|大阪市区酒店-铃鹿市区/i.test(body)) {
  console.log('【将要】恢复草稿 大阪市区酒店-铃鹿市区');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /大阪市区酒店-铃鹿市区|오사카 시내 호텔 ↔ 스즈카 시내/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.osu-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.osu-draft-id', import.meta.url), m[1]);
process.exit(0);
