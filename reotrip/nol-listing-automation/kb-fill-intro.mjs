...
const DRAFT = process.env.KB_DRAFT || readFileSync(join(__dir, '.kb-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/kobe-kix';
const IMAGES = ['kb-1.jpg', 'kb-2.jpg', 'kb-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '고베 시내 호텔';
...
const INTRO = `이 서비스는 ${A}(Kobe city hotel)와 ${B}(Kansai International Airport) 사이의 편도 전용 차량 이동 서비스입니다.
...
고베 시내 호텔 인근 관광 후 간사이 공항으로 이동하시거나, KIX 도착 후 고베 시내 호텔로 이동하시는 분들께 적합합니다.`;
...
const textOk = await page.evaluate(() => ({
  hl: /고베|간사이|KIX|Nara|Kobe|Kansai/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /고베|간사이|Nara|Kobe|Kansai|KIX/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Kobe-KIX');
...
if (g.saveDisabled) {
  ...
  console.log('【读回】upload gate final', g);
}
