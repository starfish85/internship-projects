/**
 * POP MART City Park (POPLAND) Admission Ticket
 * Excel 国内景区门票 R27: 一日票 · 只卖成人
 * 售价：只贴表格「售价hkd」列显示值；禁止用加价逻辑自算（skill 0h）
 * Family A style: adult only, 1 option, email ticket, passport+name
 * Never 提交审核
 *
 * usage: node list-popland-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 */
...
const DRAFT_FILE = join(__dir, '.popland-draft-id');
const PHASE = process.argv[2] || 'all';
// 硬规则 0h：只允许表内显示售价，禁止自算
// 【售价来源】售价hkd=180（表显示/用户截图；加价逻辑=144.09 未用；未自算；禁 180.11）
const PRICE = '180';
const PRODUCT_KO = '팝마트 시티파크 (POPLAND) 1일 입장권';
const INTERNAL = 'POP MART City Park (POPLAND) Admission Ticket';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/popland';
const IMAGES = ['popland-1.jpg', 'popland-2.jpg', 'popland-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '베이징 팝마트 시티파크(POPLAND)에서 하루 종일 캐릭터 테마파크를 즐겨보세요!';
const HIGHLIGHT = [
  'POPLAND 1일 입장권 · 성인 전용',
  '캐릭터 테마 어트랙션·포토존 체험',
  '이메일로 이용 안내/전자티켓 수령',
].join('\n');
const INTRO = `베이징 팝마트 시티파크(POP MART City Park, POPLAND) 1일 입장권입니다.
하루 동안 파크 내 어트랙션과 캐릭터 포토존을 자유롭게 이용할 수 있습니다.

포함 사항:
- POPLAND 1일 입장권 1매 (성인)
...
function readDraft() {
  if (process.env.POPLAND_DRAFT) return process.env.POPLAND_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=180（表显示；加价逻辑=144.09 未用作售价；未自算）');
  ...
  if (/POPLAND|팝마트 시티파크|POP MART City Park/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /POPLAND|팝마트 시티|POP MART City/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /朝阳|朝陽|차오양|Chaoyang|POPLAND|泡泡|通州|北京|Beijing|公园|公園/.test(t)
...
async function attrs(page) {
  ...
  // POI — 已验证：POPLAND 搜常出韩国门店；用 차오양 공원 / Chaoyang Park
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 차오양 공원 / Chaoyang（禁默认第1条韩国店）');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['차오양 공원', 'Chaoyang Park', '朝阳公园', 'POPLAND 北京'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const hit =
            items.find((e) => {
              const s = e.innerText || '';
              if (/한국|Korea|首尔|서울|호텔|Hotel|酒店/i.test(s) && !/北京|Beijing|朝阳|朝陽|차오양|通州|中国|中國/i.test(s))
                return false;
              return /朝阳|朝陽|차오양|Chaoyang|北京|Beijing|通州|中国|中國|POPLAND|泡泡/i.test(s);
            }) || null;
          if (hit) {
            hit.click();
...
async function regs(page) {
  ...
  try {
    ...
    if ((await writeInc.count()) > 0) {
      ...
      if ((await desc.count()) > 0) await desc.fill('POPLAND 1일 입장권').catch(() => {});
      await page.evaluate(() => {
...
async function options(page) {
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('POPLAND 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
  ...
  const final = await page.evaluate(() => {
    ...
    return {
      ...
      priceHit: (t.match(/180\.11/g) || []).length,
      snip: t.slice(0, 700),
    };
  });
