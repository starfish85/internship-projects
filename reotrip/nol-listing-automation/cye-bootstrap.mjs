/**
 * Excel(11) R220–223 · 7/10 售价4390/4729 · CTS-羊蹄山 航班 · 车图
 * Excel(11) R220–223 · 7/10 售价4390/4729 · CTS-羊蹄山 航班 · 车图
 * Excel(11) R220–223 · 7/10 售价4390/4729 · CTS-羊蹄山 航班 · 车图
 */
...
const PRODUCT_KO = '신치토세 공항(CTS) ↔ 요테이산 단독 차량 편도 이동 서비스';
const INTERNAL = '新千岁机场(CTS)-羊蹄山';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 新千岁机场(CTS)-羊蹄山 草稿');
killPeerCdpScripts('cye-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /신치토세.*요테이|新千岁机场\(CTS\)-羊蹄山|羊蹄山|Yotei/i.test(body)) {
  console.log('【将要】恢复草稿 新千岁机场(CTS)-羊蹄山');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /新千岁机场\(CTS\)-羊蹄山|新千岁.*迪士尼|신치토세 공항\(CTS\) ↔ 도쿄 교토/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.cye-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.cye-draft-id', import.meta.url), m[1]);
process.exit(0);
