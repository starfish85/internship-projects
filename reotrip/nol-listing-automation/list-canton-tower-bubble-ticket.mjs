/**
 * Canton Tower 460M Bubble Tram Combo
 * Excel/CSV 国内景区门票 R32: 460M 气泡缆车 combo · 只卖成人
 * 售价：只贴表格「售价hkd」列显示值；禁止用加价逻辑 337.22 自算（skill 0h）
 * Family A style: adult only, 1 option, email ticket, passport+name
 * Never 提交审核
 *
 * usage: node list-canton-tower-bubble-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 */
...
const DRAFT_FILE = join(__dir, '.canton-tower-bubble-draft-id');
const PHASE = process.argv[2] || 'all';
// 硬规则 0h：只允许表内显示售价，禁止自算 467/0.8
// 【售价来源】售价hkd=422（CSV R32 表显示；加价逻辑=337.22 未用；未自算）
const PRICE = (process.env.CANTON_TOWER_BUBBLE_PRICE || process.argv[3] || '422').trim();
const PRODUCT_KO = '광저우 캔톤타워 460M 버블트램 콤보 1일 입장권';
const INTERNAL = 'Canton Tower 460M Bubble Tram Combo';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/canton-tower-bubble';
const IMAGES = ['bubble-1.jpg', 'bubble-2.jpg', 'bubble-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '광저우 캔톤타워 460M 버블트램과 전망을 한 번에 즐겨보세요!';
const HIGHLIGHT = [
  '캔톤타워 460M 버블트램 콤보 1일 입장권 · 성인 전용',
  '450M 전망대 + 460M 버블트램 체험',
  '이메일로 이용 안내/전자티켓 수령',
].join('\n');
const INTRO = `광저우 캔톤타워(Canton Tower) 460M 버블트램 콤보 1일 입장권입니다.
450M 전망대 입장과 460M 높이의 버블트램(투명 캐빈) 체험이 포함된 콤보 상품입니다.

포함 사항:
- 캔톤타워 450M 전망대 입장 + 460M 버블트램 콤보 1매 (성인)

이용 안내:
- 티켓/이용 안내는 이용일 기준 보통 7일 이내에 예약 시 입력한 이메일로 발송됩니다. 이메일 주소가 정확하고 첨부파일을 수신할 수 있는지 확인해 주세요.
- 모든 이용자의 영문 성명과 여권번호를 예약 시 정확히 입력해 주세요.
- 본 상품은 성인 전용입니다.
- 버블트램 운행·좌석 배정은 현장 및 이메일 안내를 따릅니다.

캔톤타워의 전망과 버블트램을 함께 즐기고 싶은 분들께 추천합니다.`;
...
function readDraft() {
  if (process.env.CANTON_TOWER_BUBBLE_DRAFT) return process.env.CANTON_TOWER_BUBBLE_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=337.22 未用作售价；未自算）');
  ...
  if (/Bubble Tram|버블트램|460M|Canton Tower 460|气泡/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /Bubble Tram|버블트램|460M|Canton Tower 460/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /广州|廣州|Guangzhou|Canton|Tower|广州塔|廣州塔|캔톤|海心沙|Haixinsha|珠江|公园|公園/.test(t)
...
async function attrs(page) {
  ...
  if (!st.hasTheme) {
    ...
    if ((await themeBtn.count()) > 0) {
      ...
      let picked = false;
      for (const p of ['观景台', '觀景台', '观光', '觀光', '景点', '景點', '体验', '體驗', '액티비티', '主题公园', '主題公園', '테마파크']) {
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 广州塔 / Canton Tower');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['广州塔', 'Canton Tower', '广州塔 Canton Tower', 'Guangzhou Canton Tower'];
      let clicked = null;
...
async function regs(page) {
  ...
  try {
    ...
    if ((await writeInc.count()) > 0) {
      ...
      if ((await desc.count()) > 0) await desc.fill('캔톤타워 460M 버블트램 콤보 1일 입장권').catch(() => {});
      await page.evaluate(() => {
...
async function options(page) {
  if (!PRICE) {
    ...
    console.log('请设置: CANTON_TOWER_BUBBLE_PRICE=<表显示> node list-canton-tower-bubble-ticket.mjs options');
    process.exit(2);
  }
  const id = readDraft();
  if (!id) throw new Error('no draft');
