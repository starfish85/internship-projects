...
const PRODUCT_KO = '고베 시내 호텔 ↔ 간사이 국제공항(KIX) 단독 차량 편도 이동 서비스';
const INTERNAL = '神户市区酒店-关西国际机场';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 神户市区酒店-关西国际机场 草稿');
killPeerCdpScripts('kb-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /神户市区酒店\-关西国际机场/i.test(body)) {
  console.log('【将要】恢复草稿 神户市区酒店-关西国际机场');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /神户市区酒店\-关西国际机场|고베\ 시내\ 호텔\ ↔\ 간사이\ 국제공항\(/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.kb-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.kb-draft-id', import.meta.url), m[1]);
process.exit(0);
