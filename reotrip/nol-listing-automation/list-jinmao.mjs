 ...
 */
/**
 * 上海市区酒店 ↔ Shanghai JinMao Tower 88F 观景台 私人接送
 * Excel 国内接送产品 rows 35-38（公式求值，勿另算）：
 * 5: 213 / 304 / 425
 * 7 go: 250 / 357 / 510
 * 7 rtn: 250 / 357 / 500
 * 时段 08:00–21:30 ×28；停选项列表；永不提交审核
 */
...
const PRODUCT_KO =
  '상하이 시내 호텔 ↔ 진마오 타워 88층 전망대 단독 차량 편도 이동 서비스';
const INTERNAL = '上海市区酒店-Shanghai JinMao Tower 88th Floor Observation Deck Ticket';
const DEST_KO = '진마오 타워 88층 전망대';
const DEST_KO_SHORT = '진마오 타워';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/jinmao';
const IMAGES = ['jinmao-1.jpg', 'jinmao-2.jpg', 'jinmao-3.jpg']
...
const INTRO = `이 서비스는 상하이 시내 호텔과 ${DEST_KO}(Jinmao Tower 88th Floor Observation Deck) 사이의 편도 전용 차량 이동 서비스입니다.
...
가족, 커플, 소규모 그룹 등 진마오 타워 전망대를 방문하시는 분들께 적합합니다.
지금 바로 예약하고, 복잡한 교통 걱정 없이 전망 일정을 여유롭게 즐겨보세요!`;
...
async function bootstrap(page) {
  ...
  if (body.includes('진마오 타워') || body.includes('Top of Shanghai')) {
    console.log('Resume existing draft');
    const card = page.locator('div[class*="slot___StyledContainer4"]').filter({ hasText: /진마오 타워|Top of Shanghai/ }).first();
  ...
  await page.evaluate(() => {
    Array.from(document.querySelectorAll('button,a,div'))
      .find((b) => /新产品注册|新產品註冊|신상품/.test(b.innerText || ''))
      ?.click();
  });
  await sleep(1500);
  const inputs = page.locator('input:visible');
  const n = await inputs.count();
  await inputs.nth(n > 1 ? 1 : 0).fill(PRODUCT_KO);
  await sleep(400);
  await page.evaluate(() => {
  ...
  await page.evaluate(() => {
    Array.from(document.querySelectorAll('button'))
      .find((b) => /开始.*产品|開始.*產品|創建产品|创建產品|상품 만들기/.test(b.innerText || ''))
      ?.click();
  });
...
async function fillAttrs(page) {
  ...
  // POI Shanghai Tower / Top of Shanghai
  const hasPoi = await page.evaluate(() =>
    /金茂|Jinmao|Jin Mao|진마오|浦东|Lujiazui|世纪大道|观景/i.test(document.body.innerText),
  );
  if (!hasPoi) {
    await page.evaluate(() => {
      Array.from(document.querySelectorAll('button'))
    ...
    const picked = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('div')).filter((el) => {
        ...
        return (
          ...
          r.y > 100 &&
          r.y < 850 &&
          /金茂|Jinmao|Jin Mao|JinMao|浦东|Lujiazui|世纪大道|上海|中国|中华/i.test(t) &&
          !/서울|대한민국/.test(t)
        );
      });
...
async function holidays(page) {
  ...
  for (let oi = 0; oi < 4; oi++) {
    ...
    for (const seg of segs) {
      ...
      await page.getByRole('button', { name: /销售日历管理/ }).nth(oi).click({ force: true });
      await sleep(1800);
      await page.evaluate(() => {
        Array.from(document.querySelectorAll('button'))
...
async function main() {
  ...
  if (id) fs.writeFileSync('/tmp/nol-jinmao-id.txt', id);
