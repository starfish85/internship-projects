/**
 * 羽田机场-横滨港 介绍：韩文 + 用户3图（邮轮码头/横滨港/羽田）→ 保存然后
 * 机场+港口：航班 + 선박명
 */
import fs from 'fs';
...
const DRAFT = process.env.HYP_DRAFT || readFileSync(join(__dir, '.hyp-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/haneda-yokohama-port';
const IMAGES = ['hyp-1.jpg', 'hyp-2.jpg', 'hyp-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '하네다 공항(HND)';
const B = '요코하마항';
...
const INTRO = `이 서비스는 ${A}(Haneda / Tokyo International Airport)와 ${B}(Yokohama Port) 사이의 편도 전용 차량 이동 서비스입니다.
...
크루즈·페리 이용 및 하네다 공항 출도착 고객께 적합합니다.`;
...
const textOk = await page.evaluate(() => ({
  hl: /하네다|HND|요코하마/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /하네다|Haneda|요코하마|Yokohama/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Haneda-Yokohama');
