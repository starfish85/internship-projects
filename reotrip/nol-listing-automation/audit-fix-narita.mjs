...
const PRODUCT_KO = '도쿄 시내 호텔 ↔ 나리타공항(NRT) 단독 차량 편도 이동 서비스';
const PRODUCT_CN = '东京市区-成田机场(NRT)';
// will resolve from list if unknown
let PRODUCT_ID = process.env.NRT_ID || '60557c54-6c11-4b0e-9e04-df85c0d3e78b';
...
const OPTIONS = [
  {
    match: '도쿄 시내 호텔 출발 → 나리타공항(NRT) 편도 이동 (7인승 차량)',
    fullName: '도쿄 시내 호텔 출발 → 나리타공항(NRT) 편도 이동 (7인승 차량)',
    ptName: '7인승 가는',
    ptDesc: '7인승 차량',
    price: '112',
  },
  {
    match: '도쿄 시내 호텔 출발 → 나리타공항(NRT) 편도 이동 (10인승 차량)',
    fullName: '도쿄 시내 호텔 출발 → 나리타공항(NRT) 편도 이동 (10인승 차량)',
    ptName: '10인승 가는',
    ptDesc: '10인승 차량',
    price: '175',
  },
  {
    match: '나리타공항(NRT) 출발 → 도쿄 시내 호텔 편도 이동 (7인승 차량)',
    fullName: '나리타공항(NRT) 출발 → 도쿄 시내 호텔 편도 이동 (7인승 차량)',
    ptName: '7인승 오는',
    ptDesc: '7인승 차량',
    price: '112',
  },
  {
    match: '나리타공항(NRT) 출발 → 도쿄 시내 호텔 편도 이동 (10인승 차량)',
    fullName: '나리타공항(NRT) 출발 → 도쿄 시내 호텔 편도 이동 (10인승 차량)',
    ptName: '10인승 오는',
    ptDesc: '10인승 차량',
    price: '175',
  },
...
async function resolveProductId(page) {
  ...
  if (body.includes('나리타') || body.includes(PRODUCT_KO) || body.includes('成田')) {
  ...
  const clicked = await page.evaluate((names) => {
    ...
    // broader: any div with product name
    ...
    return null;
  }, [PRODUCT_KO, '나리타공항(NRT)', PRODUCT_CN, '成田']);
...
async function audit(page) {
  ...
  report.titleOk = body.includes('나리타') || body.includes('成田');
...
async function main() {
  ...
  if (!okId) throw new Error('Cannot resolve Narita product id');
