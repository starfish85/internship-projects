 ...
 * CSV 韩国门票产品 R5: 一日票 · 只卖成人 · 外籍特惠（不支持韩国国籍）
 * 【售价来源】售价hkd=113（表显示；加价逻辑 90 未用；未自算）skill 0h
 * 单选项 1일 입장권；0n 双门禁 代表+按数量
 * 串行：本脚本只上 Nami Island，禁并行其他
 * Never 提交审核
 *
 * usage: node list-nami-island-ticket.mjs [bootstrap|attrs|intro|regs|options]
 */
...
const DRAFT_FILE = join(__dir, '.nami-island-draft-id');
const PHASE = process.argv[2] || 'bootstrap';
// 【售价来源】售价hkd=252（CSV 韩国门票 R5 表显示；加价逻辑=90 未用；未自算）
const PRICE = (process.env.NAMI_ISLAND_PRICE || process.argv[3] || '113').trim();
const PRODUCT_KO = '남이섬 1일 입장권';
const INTERNAL = 'Nami Island / 남이섬 1日票';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/nami-island';
const IMAGES = ['ni-1.jpg', 'ni-2.jpg', 'ni-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '남이섬에서 자연과 산책로를 즐기세요!';
const HIGHLIGHT = [
  '남이섬 1일 입장권 · 성인 전용',
  '봄꽃·메타세쿼이아 산책 · 이메일 전자티켓/이용 안내',
  '예약 시 전화·이메일·영문성명·여권번호 필수',
].join('\n');
const INTRO = `남이섬(Nami Island) 1일 입장권입니다.
춘천 인근의 섬 산책 명소로, 사계절 자연 경관과 산책로를 즐길 수 있습니다.
...
function readDraft() {
  if (process.env.NAMI_ISLAND_DRAFT) return process.env.NAMI_ISLAND_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=90 未用作售价；未自算）');
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /남이섬|Nami|南怡|춘천|Chuncheon|가평|Gapyeong/.test(t)
...
async function attrs(page) {
  ...
  if (!st.hasTheme) {
    ...
    if ((await themeBtn.count()) > 0) {
      ...
      picked = await page.evaluate(() => {
        ...
        if (!dlg) return false;
        const want = ['景点', '景點', '旅游地', '旅遊地', '体验', '观光', '觀光'];
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 남이섬 / Nami Island');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['Nami Island', '남이섬', 'Nami', '南怡岛'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const score = (s) => {
            if (/Hotel|호텔|酒店|숙박|住宿/i.test(s) && !/남이|Nami/i.test(s)) return -50;
            if (/广州|廣州|北京|上海|Macau|에버랜드|Everland|롯데/i.test(s)) return -50;
            if (/남이섬|Nami Island|Nami/i.test(s) && /춘천|Chuncheon|가평|Gapyeong|경기|Gyeonggi|韩国|Korea/i.test(s)) return 100;
            if (/남이섬|Nami Island|Nami/i.test(s)) return 85;
            return 0;
          };
          let best = null;
          let bs = 0;
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '남이섬 1일 입장권' }],
...
async function options(page) {
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('남이섬 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
