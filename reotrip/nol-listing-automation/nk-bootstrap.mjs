...
const PRODUCT_KO = '나라 시내 호텔 ↔ 간사이 국제공항(KIX) 단독 차량 편도 이동 서비스';
const INTERNAL = '奈良市区酒店-关西国际机场';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 奈良市区酒店-关西国际机场 草稿');
killPeerCdpScripts('nk-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /奈良市区酒店\-关西国际机场/i.test(body)) {
  console.log('【将要】恢复草稿 奈良市区酒店-关西国际机场');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /奈良市区酒店\-关西国际机场|나라\ 시내\ 호텔\ ↔\ 간사이\ 국제공항\(/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.nk-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.nk-draft-id', import.meta.url), m[1]);
process.exit(0);
