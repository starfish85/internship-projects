...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/nara-kix';
const IMAGES = ['nk-1.jpg', 'nk-2.jpg', 'nk-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '나라 시내 호텔';
...
const INTRO = `이 서비스는 ${A}(Nara city hotel)와 ${B}(Kansai International Airport) 사이의 편도 전용 차량 이동 서비스입니다.
...
나라 시내 호텔 인근 관광 후 간사이 공항으로 이동하시거나, KIX 도착 후 나라 시내 호텔로 이동하시는 분들께 적합합니다.`;
...
const textOk = await page.evaluate(() => ({
  hl: /나라|간사이|KIX|Nara|Kobe|Kansai/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /나라|간사이|Nara|Kobe|Kansai|KIX/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Nara-KIX');
