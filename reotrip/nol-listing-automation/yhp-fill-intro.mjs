/**
 * 横滨市区酒店-横滨港 介绍：韩文文案 + 썸네일 3 图 → 保存然后
 * 港口产品：船名/승하선，无航班
 */
import fs from 'fs';
...
const DRAFT = process.env.YHP_DRAFT || readFileSync(join(__dir, '.yhp-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/yokohama-hotel-port';
const IMAGES = ['yhp-1.jpg', 'yhp-2.jpg', 'yhp-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter(fs.existsSync);
const CITY = '요코하마 시내 호텔';
const DEST = '요코하마항';
...
const INTRO = `이 서비스는 ${CITY}과 ${DEST}(요코하마항 / Yokohama Port) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 호텔명·주소, 요코하마항 내 승하차 또는 픽업 장소, 이용 시간, 연락 가능한 휴대전화 번호, 선박명 및 승하선 정보, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 크루즈/페리 이용 등 요코하마항를 이용하시는 분들께 적합합니다.`;
...
2.미팅 장소: 호텔 주소와 요코하마항 픽업/샌딩 장소·이용 시간, 선박명 및 승하선 정보를 정확히 입력해 주세요.
...
3.예약 시 호텔 주소, 요코하마항 픽업/샌딩 장소, 선박명·승하선 시간, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('yhp-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /요코하마|Yokohama/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /요코하마|Yokohama/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
