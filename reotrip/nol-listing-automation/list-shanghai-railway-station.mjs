/**
 * 上海市区酒店 ↔ 上海火车站 私人接送（上海站（主站），非虹桥/机场）
...
const INTRO = `이 서비스는 ${CITY_KO}과 ${DEST_KO}(上海火车站 / Shanghai Railway Station) 사이의 편도 전용 차량 이동 서비스입니다.
...
async function getPage() {
  ...
  return connectNolPage({
    selfHint: 'list-shanghai-railway-station',
    killPeers: true,
    forceViewport: true,
...
async function bootstrap(page) {
  ...
  if (body.includes(PRODUCT_KO) || body.includes(INTERNAL) || body.includes('상하이역 단독')) {
    console.log('Resume existing draft');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /상하이역 단독|上海市区酒店-上海火车站|上海火车站/ })
...
async function fillAttrs(page) {
  ...
  // POI 上海火车站（火车站，排除机场）— §43 元素定位
  const hasPoi = await page.evaluate(() =>
    /火车站|上海站|上海火车站|상하이역|Shanghai Railway Station|Shanghai Station/i.test(document.body.innerText) &&
      !/虹桥|Hongqiao|浦东|Pudong|机场|Airport|广州|Guangzhou/i.test(document.body.innerText.slice(0, 500)),
  );
  if (!hasPoi) {
    ...
    let clicked = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('div,li,button')).filter((el) => {
        ...
        return (
          ...
          r.height < 200 &&
          r.y > 80 &&
          /火车站|上海站|Railway Station|상하이역|Shanghai Station|上海火车站/i.test(t) &&
          /站|Station|역|铁路|Railway/i.test(t) &&
          /上海|Shanghai|中国|中华/i.test(t) &&
          !/机场|Airport|浦东|Pudong|北京|大兴|虹桥|Hongqiao|广州|Guangzhou/i.test(t)
        );
      });
    ...
    if (!clicked) {
      await search.fill('Shanghai Railway Station');
      await page.keyboard.press('Enter');
      await sleep(2200);
      clicked = await page.evaluate(() => {
        const cards = Array.from(document.querySelectorAll('div,li,button')).filter((el) => {
          ...
          return (
            ...
            r.height > 40 &&
            r.height < 200 &&
            /Shanghai|上海/i.test(t) &&
            /Station|Railway|火车站|上海站|역/i.test(t) &&
            !/Airport|机场|Hongqiao|虹桥|Pudong|浦东|Guangzhou|广州/i.test(t)
          );
        });
        if (!cards[0]) return null;
        cards[0].click();
