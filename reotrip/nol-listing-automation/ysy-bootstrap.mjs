/**
 * 横滨港-新横滨站 — 创建或恢复草稿
 * Excel(11) R164–167 · 7/10 成本→售价1053/1389 · 港口无航班 日本接送 R86–89 · 7/10 成本737.25/972.55 → 售价1053/1389 · 无假日
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '요코하마항 ↔ 신요코하마역 단독 차량 편도 이동 서비스';
const INTERNAL = '横滨港-新横滨站';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 横滨港-新横滨站 草稿');
killPeerCdpScripts('ysy-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /요코하마항.*신요코하마역|横滨港-新横滨站/i.test(body)) {
  console.log('【将要】恢复草稿 横滨港-新横滨站');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /横滨港-新横滨站|요코하마항 ↔ 신요코하마역/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.ysy-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.ysy-draft-id', import.meta.url), m[1]);
process.exit(0);
