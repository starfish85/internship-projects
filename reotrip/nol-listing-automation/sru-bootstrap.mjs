/**
 * Excel(11) R224–227 · 7/10 售价3476/3950 · 札幌-留寿都 无航班 · 景区+车图
 * Excel(11) R224–227 · 7/10 售价3476/3950 · 札幌-留寿都 无航班 · 景区+车图
 */
...
const PRODUCT_KO = '삿포로 시내 ↔ 루스쓰 단독 차량 편도 이동 서비스';
const INTERNAL = '札幌市区-留寿都';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 札幌市区-留寿都 草稿');
killPeerCdpScripts('sru-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /삿포로.*루스쓰|札幌市区-留寿都|留寿都|Rusutsu/i.test(body)) {
  console.log('【将要】恢复草稿 札幌市区-留寿都');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /札幌市区-留寿都|삿포로 시내 ↔ 루스쓰/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.sru-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.sru-draft-id', import.meta.url), m[1]);
process.exit(0);
