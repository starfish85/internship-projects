/**
 * 羽田机场(HND)-东京迪士尼 — 创建或恢复草稿
 * Excel(11) R168–171 · 7/10 成本→售价784/1176 · 机场航班预约 日本接送 Excel(10) R107–110 · 7/10 售价1580/2054 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '하네다 공항(HND) ↔ 도쿄 디즈니 리조트 단독 차량 편도 이동 서비스';
const INTERNAL = '羽田机场(HND)-东京迪士尼';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 羽田机场(HND)-东京迪士尼 草稿');
killPeerCdpScripts('htd-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /하네다.*디즈니|羽田机场\(HND\)-东京迪士尼|羽田.*迪士尼/i.test(body)) {
  console.log('【将要】恢复草稿 羽田机场(HND)-东京迪士尼');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /羽田机场\(HND\)-东京迪士尼|羽田.*迪士尼|하네다 공항\(HND\) ↔ 도쿄 디즈니/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.htd-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.htd-draft-id', import.meta.url), m[1]);
process.exit(0);
