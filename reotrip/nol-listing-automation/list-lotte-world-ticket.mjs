/**
 * 乐天世界 Lotte World Admission Ticket
 * CSV 韩国门票产品 R1: 一日票 · 只卖成人 · 外籍特惠（不支持韩国国籍）
 * 【售价来源】售价hkd=252（表显示；加价逻辑 202 未用；未自算）skill 0h
 * 单选项 1일 입장권；0n 双门禁 代表+按数量
 * 串行：本脚本只上乐天世界，禁并行乐天塔
 * Never 提交审核
 *
 * usage: node list-lotte-world-ticket.mjs [bootstrap|attrs|intro|regs|options]
 */
...
const DRAFT_FILE = join(__dir, '.lotte-world-draft-id');
const PHASE = process.argv[2] || 'bootstrap';
// 【售价来源】售价hkd=252（CSV 韩国门票 R1 表显示；加价逻辑=202 未用；未自算）
const PRICE = (process.env.LOTTE_WORLD_PRICE || process.argv[3] || '252').trim();
const PRODUCT_KO = '롯데월드 1일 입장권 (외국인 특혜)';
const INTERNAL = '乐天世界 Lotte World 1日票 外籍特惠';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/lotte-world';
const IMAGES = ['lw-1.jpg', 'lw-2.jpg', 'lw-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '서울 롯데월드에서 하루 종일 어드벤처를 즐기세요!';
const HIGHLIGHT = [
  '롯데월드 1일 입장권 · 성인 전용 · 외국인 특혜 요금',
  '어드벤처 테마파크 종일 이용 · 이메일 전자티켓/이용 안내',
  '한국 국적자 구매 불가 · 예약 시 전화·이메일·여권 정보 필수',
].join('\n');
const INTRO = `롯데월드(Lotte World) 어드벤처 1일 입장권입니다.
서울 잠실의 대표 테마파크에서 다양한 어트랙션을 하루 종일 즐길 수 있습니다.

포함 사항:
- 롯데월드 어드벤처 1일 입장권 1매 (성인)
...
서울 테마파크 어드벤처를 즐기고 싶은 외국인 이용객께 추천합니다.`;
...
function readDraft() {
  if (process.env.LOTTE_WORLD_DRAFT) return process.env.LOTTE_WORLD_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=202 未用作售价；未自算）');
  ...
  // resume 必须同时含 롯데월드/Lotte World + 입장권；禁误点 롯데타워/乐天塔
  if (/롯데월드|Lotte World|乐天世界/i.test(body) && !/롯데타워|Lotte Tower|乐天塔/i.test(body.split('\n').find((l) => /롯데|Lotte|乐天/.test(l)) || '')) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /롯데월드|Lotte World|乐天世界/i })
      .filter({ hasNotText: /롯데타워|Lotte Tower|乐天塔/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /乐天|樂天|Lotte|롯데|잠실|Jamsil|서울|Seoul/.test(t)
...
async function attrs(page) {
  ...
  if (!st.hasTheme) {
    ...
    if ((await themeBtn.count()) > 0) {
      ...
      picked = await page.evaluate(() => {
        ...
        if (!dlg) return false;
        const want = ['主题公园', '主題公園', '테마파크', '体验', '景点', '景點'];
  ...
  // POI — 首尔 乐天世界（禁 乐天塔/酒店/中国错城）
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 롯데월드 / Lotte World Seoul');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['Lotte World', '롯데월드', '乐天世界', 'Lotte World Adventure Seoul', '잠실 롯데월드'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          // 优先 롯데월드/Lotte World + 서울/Seoul/잠실；拒 롯데타워/酒店/中国
          const hit =
            items.find((e) => {
              const s = e.innerText || '';
              if (/롯데타워|Lotte Tower|乐天塔|Hotel|호텔|酒店|숙박|住宿/i.test(s) && !/Adventure|어드벤처|테마파크|Theme/i.test(s))
                return false;
              if (/广州|廣州|北京|上海|Macau|澳门|澳門/i.test(s)) return false;
              return /롯데월드|Lotte World|乐天世界|Adventure/i.test(s) && /서울|Seoul|잠실|Jamsil|Songpa|松坡|韩国|韓國|Korea/i.test(s);
            }) ||
            items.find((e) => {
              const s = e.innerText || '';
              if (/롯데타워|Lotte Tower|Hotel|호텔|酒店/i.test(s)) return false;
              return /롯데월드|Lotte World|乐天世界/i.test(s);
            }) ||
            null;
          if (hit) {
            hit.click();
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '롯데월드 1일 입장권' }],
...
async function options(page) {
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('롯데월드 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
