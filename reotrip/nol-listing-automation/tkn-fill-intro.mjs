...
const DRAFT = process.env.TKN_DRAFT || readFileSync(join(__dir, '.tkn-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/tokyo-nagano';
const IMAGES = ['tkn-1.jpg', 'tkn-2.jpg', 'tkn-3.jpg']
...
const HIGHLIGHT = [
  ...
  '장거리 이동도 프라이빗하게, 환승 없이 도어 투 도어',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${CITY}(도쿄 / Tokyo)와 ${DEST}(나가노 / Nagano) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 도쿄 호텔명·주소 또는 픽업 장소, 나가노 호텔·역·관광지 등 샌딩 장소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

스키·온천·고원 여행, 가족·소규모 그룹 등 도쿄와 나가노를 편안하게 오가시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 도쿄 픽업 장소·주소와 나가노 샌딩 장소·이용 시간을 정확히 입력해 주세요.
...
const textOk = await page.evaluate(() => ({
  hl: /도쿄|나가노|Tokyo|Nagano|東京|長野/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /도쿄|나가노|Tokyo|Nagano|東京|長野/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
