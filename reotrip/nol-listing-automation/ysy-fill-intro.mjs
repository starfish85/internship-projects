...
const DRAFT = process.env.YSY_DRAFT || readFileSync(join(__dir, '.ysy-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/yokohama-port-shin-yokohama';
const IMAGES = ['ysy-1.jpg', 'ysy-2.jpg', 'ysy-3.jpg']
...
const INTRO = `이 서비스는 ${CITY}(요코하마항 / Yokohama Port · 오산바시 등)과 ${DEST}(신요코하마역 / Shin-Yokohama Station) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 요코하마항(승하선 터미널) 픽업/샌딩 장소, 신요코하마역 승하차 위치, 이용 시간, 연락 가능한 휴대전화 번호, 선박명 및 승하선 정보, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 크루즈/신칸센 환승 등 요코하마항과 신요코하마역을 오가시는 분들께 적합합니다.`;
...
2.미팅 장소: 요코하마항 픽업/샌딩 장소와 신요코하마역 승하차 위치·이용 시간, 선박명 및 승하선 정보를 정확히 입력해 주세요.
...
3.예약 시 호텔 주소, 신요코하마역 픽업/샌딩 장소, 선박명·승하선 시간, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('ysy-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /요코하마|신요코하마역|Yokohama|Tokyo/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /요코하마|신요코하마역|Yokohama|Tokyo|크루즈|Cruise/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Yokohama Port-Shin-Yokohama');
