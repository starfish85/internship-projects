/**
 * 成田机场-横滨港 — 创建或恢复草稿
 * Excel (9) 日本接送 R82–85 · 7/10 成本1129.41/1364.71 → 售价1613/1950 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '나리타 공항(NRT) ↔ 요코하마항 단독 차량 편도 이동 서비스';
const INTERNAL = '成田机场-横滨港';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 成田机场-横滨港 草稿');
killPeerCdpScripts('nyp-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /나리타.*요코하마|成田机场-横滨港/i.test(body)) {
  console.log('【将要】恢复草稿 成田机场-横滨港');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /成田机场-横滨港|나리타 공항.*요코하마항/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.nyp-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.nyp-draft-id', import.meta.url), m[1]);
process.exit(0);
