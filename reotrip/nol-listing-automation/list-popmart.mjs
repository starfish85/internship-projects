...
/**
 * 北京市区酒店 ↔ 北京泡泡马特 私人接送
 * Excel 国内接送 R44-47 公式求值（勿另算，勿抄上海 510/500）：
 * 5: 成本175 → 平日219 / 五一十一313 / 春节438
 * 7: 成本250 → 平日313 / 五一十一446 / 春节625（去返对称 F=D/0.5）
 * 时段 08:00–21:30 ×28；Verify-Before-Next §40；停选项列表；永不提交审核
 */
...
const PRODUCT_KO =
  '베이징 시내 호텔 ↔ 베이징 팝마트 단독 차량 편도 이동 서비스';
const INTERNAL = '北京市区酒店-北京泡泡马特';
const DEST_KO = '베이징 팝마트';
const DEST_KO_SHORT = '베이징 팝마트';
const CITY_KO = '베이징 시내 호텔';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/popmart';
const IMAGES = ['popmart-1.jpg', 'popmart-2.jpg', 'popmart-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

// Excel 售价列求值整数（round）
const PRICES = {
  '5go': { normal: '219', oct: '313', spring: '438', may: '313' },
  '7go': { normal: '313', oct: '446', spring: '625', may: '446' },
  '5rtn': { normal: '219', oct: '313', spring: '438', may: '313' },
  '7rtn': { normal: '313', oct: '446', spring: '625', may: '446' },
};

const OPTIONS = [
  {
    key: '5go',
    name: `${CITY_KO} 출발 → ${DEST_KO} 편도 이동 (5인승 차량)`,
    desc: `${CITY_KO} 출발 → ${DEST_KO} 편도 이동 (5인승 차량, 최대 4인 탑승 가능)\n24인치(표준) 이하 수하물 기준: 59cm(높이) × 41cm(너비) × 24cm(두께) 이내\n5인승 차량: 최대 2개까지 적재 가능`,
    pt: '5인승 가는',
    ptd: '5인승 차량',
  },
  {
    ...
    desc: `${CITY_KO} 출발 → ${DEST_KO} 편도 이동 (7인승 차량, 최대 6인 탑승 가능)\n24인치(표준) 이하 수하물 기준: 59cm(높이) × 41cm(너비) × 24cm(두께) 이내\n7인승 차량: 최대 3개까지 적재 가능`,
    pt: '7인승 가는',
    ptd: '7인승 차량',
  },
  {
    key: '5rtn',
    name: `${DEST_KO} 출발 → ${CITY_KO} 편도 이동 (5인승 차량)`,
    desc: `${DEST_KO} 출발 → ${CITY_KO} 편도 이동 (5인승 차량, 최대 4인 탑승 가능)\n24인치(표준) 이하 수하물 기준: 59cm(높이) × 41cm(너비) × 24cm(두께) 이내\n5인승 차량: 최대 2개까지 적재 가능`,
    pt: '5인승 오는',
    ptd: '5인승 차량',
  },
  {
    ...
    desc: `${DEST_KO} 출발 → ${CITY_KO} 편도 이동 (7인승 차량, 최대 6인 탑승 가능)\n24인치(표준) 이하 수하물 기준: 59cm(높이) × 41cm(너비) × 24cm(두께) 이내\n7인승 차량: 최대 3개까지 적재 가능`,
    pt: '7인승 오는',
    ptd: '7인승 차량',
  },
];
...
const HIGHLIGHT = [
  `${CITY_KO}에서 ${DEST_KO}까지 단독 차량으로 편안하게 이동`,
  '대중교통 환승 없이 빠르고 쾌적한 전용 픽업 서비스',
  '숙련된 기사님의 안전하고 친절한 응대',
].join('\n');
const INTRO = `이 서비스는 ${CITY_KO}과 ${DEST_KO}(北京泡泡马特 / Pop Mart) 사이의 편도 전용 차량 이동 서비스입니다.
...
async function bootstrap(page) {
  ...
  if (body.includes('베이징 팝마트') || body.includes('泡泡马特') || body.includes('팝마트')) {
    console.log('Resume existing draft');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /베이징 팝마트|泡泡马特|팝마트/ })
      .first();
...
async function fillAttrs(page) {
  ...
  // POI 北京泡泡马特 / Pop Mart / Pop Land Beijing
  const hasPoi = await page.evaluate(() =>
    /泡泡|Pop\s*Mart|POP\s*LAND|팝마트|PopMart|朝阳|北京/i.test(document.body.innerText) &&
    /泡泡|Pop|팝마트|POP LAND/i.test(document.body.innerText),
  );
  if (!hasPoi) {
    await page.evaluate(() => {
      Array.from(document.querySelectorAll('button'))
    ...
    let picked = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('div')).filter((el) => {
        ...
        return (
          ...
          r.y > 100 &&
          r.y < 850 &&
          /泡泡|Pop\s*Mart|POP\s*LAND|PopMart|玛特|马特/i.test(t) &&
          /北京|Beijing|中国|중华|中华|朝阳/i.test(t) &&
          !/서울|대한민국|上海|Shanghai|深圳|广州/.test(t)
        );
      });
    ...
    if (!picked) {
      await search.fill('Pop Mart Beijing');
      await page.keyboard.press('Enter');
      await sleep(2000);
...
async function createOptions(page) {
  ...
  for (const opt of OPTIONS) {
    ...
    // times 08:00-21:30 — gate: must be 28 slots before next
    let tv = await setTimes(page);
    if (!tv || tv.count !== 28 || tv.first !== '08:00' || tv.last !== '21:30') {
      console.log('times fail, retry once', tv);
      tv = await setTimes(page);
    }
    if (!tv || tv.count !== 28 || tv.first !== '08:00' || tv.last !== '21:30') {
      console.log('TIMES_FAIL', opt.key, tv);
      // still temp-save name/price; fix-tussauds-times later
    } else {
...
async function setTimes(page) {
  ...
  if (
    !(await page.evaluate(() =>
      Array.from(document.querySelectorAll('[role=dialog]')).some((d) => /设置时间|重复/.test(d.innerText || '')),
    ))
  ) {
    ...
    if ((await more.count()) > 0) {
      ...
      await sleep(400);
      await page.evaluate(() => {
        Array.from(document.querySelectorAll('div,button,span'))
          .find((el) => {
            const t = (el.innerText || '').trim();
            const r = el.getBoundingClientRect();
            return t === '编辑' && r.height > 20 && r.height < 60;
          })
          ?.click();
      });
      await sleep(1200);
    ...
    const fields = await page.evaluate(() =>
      Array.from(document.querySelectorAll('button'))
        .map((b, i) => {
          const t = (b.innerText || '').trim();
          const r = b.getBoundingClientRect();
          return { i, t, w: r.width, vis: r.height > 20 && r.y > 80 && r.y < 850 };
        })
        .filter((b) => b.vis && b.w > 100 && (b.t === '选择' || b.t === '選擇' || /^\d{2}:\d{2}$/.test(b.t))),
    );
  ...
  await page.evaluate(() => {
    Array.from(document.querySelectorAll('button'))
      .find((x) => /分钟|分鐘/.test((x.innerText || '').trim()) && x.getBoundingClientRect().y > 100)
      ?.click();
  });
  await sleep(300);
  await page.evaluate(() => {
    Array.from(document.querySelectorAll('[role=option],button,li,div'))
      .find((e) => {
        const t = (e.innerText || '').trim();
        const r = e.getBoundingClientRect();
        return t === '30' && r.width > 20 && r.height < 50 && r.y > 100;
      })
      ?.click();
  });
  await sleep(300);
...
async function main() {
  ...
  if (id) fs.writeFileSync('/tmp/nol-popmart-id.txt', id);
