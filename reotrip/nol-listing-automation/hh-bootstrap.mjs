/**
 * 箱根市区酒店-羽田机场 — 创建或恢复草稿
 * Excel (9) 日本接送 Excel(10) R91–94 · 7/10 售价3205/3544 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '하코네 시내 호텔 ↔ 하네다 공항(HND) 단독 차량 편도 이동 서비스';
const INTERNAL = '箱根市区酒店-羽田机场';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 箱根市区酒店-羽田机场 草稿');
killPeerCdpScripts('hh-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /하코네.*하네다|箱根市区酒店-羽田机场/i.test(body)) {
  console.log('【将要】恢复草稿 箱根市区酒店-羽田机场');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /箱根市区酒店-羽田机场|하코네 시내 호텔 ↔ 하네다/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.hh-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.hh-draft-id', import.meta.url), m[1]);
process.exit(0);
