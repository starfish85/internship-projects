/**
 * 中部国际机场(NGO)-吉卜力公园 — 创建或恢复草稿
 * Excel(11) R132–135 · 7/10 售价1490/1851 · 机场航班预约 日本接送 Excel(10) R107–110 · 7/10 售价1580/2054 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '중부 국제공항(NGO) ↔ 지브리 파크 단독 차량 편도 이동 서비스';
const INTERNAL = '中部国际机场(NGO)-吉卜力公园';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 中部国际机场(NGO)-吉卜力公园 草稿');
killPeerCdpScripts('cgp-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /중부.*지브리|中部国际机场\(NGO\)-吉卜力|中部国际.*吉卜力/i.test(body)) {
  console.log('【将要】恢复草稿 中部国际机场(NGO)-吉卜力公园');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /中部国际机场\(NGO\)-吉卜力|中部国际.*吉卜力|중부 국제공항\(NGO\) ↔ 지브리/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.cgp-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.cgp-draft-id', import.meta.url), m[1]);
process.exit(0);
