...
const PRODUCT_KO = '교토 시내 호텔 ↔ 교토역 단독 차량 편도 이동 서비스';
const INTERNAL = '京都市区酒店-京都站';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 京都站 草稿');
killPeerCdpScripts('ky-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /교토역|京都站|Kyoto Station|京都駅/i.test(body)) {
  console.log('【将要】恢复草稿 京都站');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /교토역|京都站|Kyoto Station|京都駅|京都市区酒店-京都站/i })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.ky-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.ky-draft-id', import.meta.url), m[1]);
process.exit(0);
