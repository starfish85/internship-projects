...
const DRAFT = process.env.TSH_DRAFT || readFileSync(join(__dir, '.tsh-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/tokyo-stn-haneda';
const IMAGES = ['tsh-1.jpg', 'tsh-2.jpg', 'tsh-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '도쿄역';
const B = '하네다 공항(HND)';
...
const INTRO = `이 서비스는 ${A}(Tokyo Station)와 ${B}(Haneda / Tokyo International Airport) 사이의 편도 전용 차량 이동 서비스입니다.
...
신칸센·열차 이용 후 하네다로 이동하시거나, 하네다 도착 후 도쿄역으로 이동하시는 분들께 적합합니다.`;
...
const textOk = await page.evaluate(() => ({
  hl: /도쿄역|하네다|HND|Tokyo|Haneda/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /도쿄역|하네다|Tokyo|Haneda|HND/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing TokyoStn-Haneda');
