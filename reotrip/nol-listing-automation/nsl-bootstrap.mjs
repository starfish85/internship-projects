/**
* 名古屋-铃鹿赛道/乐高乐园 — 创建或恢复草稿
 * Excel(11) 日本接送 R180–183 · 7/10 成本→售价1994/2465 · 景区无航班 · 无假日
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '나고야 ↔ 스즈카 서킷/레고랜드 재팬 단독 차량 편도 이동 서비스';
const INTERNAL = '名古屋-铃鹿赛道/乐高乐园';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 名古屋-铃鹿赛道/乐高乐园 草稿');
killPeerCdpScripts('nsl-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /나고야.*스즈카|나고야.*레고랜드|名古屋-铃鹿|乐高乐园|铃鹿赛道/i.test(body)) {
  console.log('【将要】恢复草稿 名古屋-铃鹿赛道/乐高乐园');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /名古屋-铃鹿|铃鹿赛道|乐高乐园|나고야 ↔ 스즈카|레고랜드 재팬/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.nsl-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.nsl-draft-id', import.meta.url), m[1]);
process.exit(0);