...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/tokyo-harry-potter';
const IMAGES = ['hp-1.jpg', 'hp-2.jpg', 'hp-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter(fs.existsSync);
const CITY = '도쿄 시내 호텔';
const DEST = '워너브라더스 스튜디오 투어 도쿄(해리포터)';
const HEADLINE = `${CITY} ↔ ${DEST} 편도 전용 차량으로 여유로운 이동을 즐기세요!`;
const HIGHLIGHT = [
  `${CITY}과 ${DEST}을 편안하게 연결하는 단독 차량 이동`,
  '스튜디오 투어 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${CITY}과 ${DEST}(워너브라더스 스튜디오 투어 도쿄 · Warner Bros. Studio Tour Tokyo – The Making of Harry Potter) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 호텔명·주소, 픽업/샌딩 장소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 소규모 그룹 등 도쿄 해리포터 스튜디오 투어를 이용하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 4개까지 적재 가능. 차량 공간을 고려해 적합한 차형을 선택해 주세요.

2.미팅 장소: 호텔 주소와 스튜디오 투어 픽업/샌딩 장소·이용 시간을 정확히 입력해 주세요.
...
3.예약 시 호텔명·주소, 목적지 픽업/샌딩 장소, 이용 시간, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('hp-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /해리포터|워너|Harry/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: (document.querySelector('#description, textarea[name=description]')?.value || '').includes('Harry') || (document.querySelector('#description, textarea[name=description]')?.value || '').includes('스튜디오'),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Harry Potter');
