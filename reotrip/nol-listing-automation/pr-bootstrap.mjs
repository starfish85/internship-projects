/**
 * 上海浦东机场(PVG) — 创建或恢复草稿
 * Excel R56–57 · 内部名 上海市区酒店-上海浦东机场
 * §55 单步 · §52 视口 · §53 杀残留
 */
...
const PRODUCT_KO =
  '상하이 푸동국제공항(PVG) ↔ 상하이역 단독 차량 편도 이동 서비스';
const INTERNAL = '上海浦东机场-上海火车站';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 浦东机场-火车站 草稿');
killPeerCdpScripts('pr-bootstrap');
...
if (
  ...
  /푸동국제공항.*상하이역|浦东机场-上海火车|PVG.*역/.test(body)
) {
  console.log('【将要】恢复草稿 浦东机场-火车站（排除虹桥/火车站）');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /푸동국제공항|浦东机场|PVG|上海市区酒店-上海浦东机场/ })
    .filter({ hasNotText: /훙차오|虹桥|Hongqiao|상하이역 단독|火车站/ })
    .first();
  if (await card.count()) {
