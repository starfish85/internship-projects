...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/sapporo-niseko';
const IMAGES = ['snk-1.jpg', 'snk-2.jpg', 'snk-3.jpg']
...
const HIGHLIGHT = [
  ...
  '삿포로·니세코 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${CITY}(삿포로 / Sapporo)과 ${DEST}(니세코 / Niseko, 北海道) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 삿포로 시내 호텔명·주소, 니세코 호텔/픽업·샌딩 장소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

스키·리조트 여행, 가족·소규모 그룹 등 삿포로 시내와 니세코를 오가시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 삿포로 시내 호텔 주소와 니세코 픽업/샌딩 장소·이용 시간을 정확히 입력해 주세요.
...
const textOk = await page.evaluate(() => ({
  hl: /삿포로|니세코|Sapporo|Niseko/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /삿포로|니세코|Sapporo|Niseko|二世谷|ニセコ/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Sapporo-Niseko');
