/**
 * 中部国际机场(NGO)-名古屋市区酒店 — 创建或恢复草稿
 * Excel(11) R136–139 · 7/10 售价1264/2054 · 机场航班预约 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '중부 국제공항(NGO) ↔ 나고야 시내 호텔 단독 차량 편도 이동 서비스';
const INTERNAL = '中部国际机场(NGO)-名古屋市区酒店';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 中部国际机场(NGO)-名古屋市区酒店 草稿');
killPeerCdpScripts('cnh-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /중부.*나고야|中部国际机场\(NGO\)-名古屋|中部国际.*名古屋/i.test(body)) {
  console.log('【将要】恢复草稿 中部国际机场(NGO)-名古屋市区酒店');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /中部国际机场\(NGO\)-名古屋|中部国际.*名古屋|중부 국제공항\(NGO\) ↔ 나고야/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.cnh-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.cnh-draft-id', import.meta.url), m[1]);
process.exit(0);
