/**
 * 成田(NRT)-东京迪士尼 法规：航班预约 + 디즈니 입장권 排除
 */
...
const CITY_KO = '나리타 공항(NRT)';
const DEST_KO = '도쿄 디즈니 리조트';
const INCLUDE_TR = `${CITY_KO} ↔ ${DEST_KO} 편도 전용 차량 이동 및 주차비 포함`;
const INCLUDE_PU = '픽업/샌딩 서비스 및 주차비 포함';
const EXCLUDE =
  '항공권, 공항 이용료, 디즈니 입장권, 가이드, 팁, 개인 비용, 아동용 카시트, 야간 할증, 추가 경유지 및 기타 명시되지 않은 비용은 포함되어 있지 않습니다.';
const RESV_IDS = [
  'CELLPHONE-required',
  'EMAIL-required',
...
killPeerCdpScripts('ntd-fill-regs');
