...
const PRODUCT_KO =
  '상하이 시내 호텔 ↔ 상하이 우숭커우 국제크루즈터미널 단독 차량 편도 이동 서비스';
const INTERNAL = '上海市区酒店-吴淞口港口';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 WSK 草稿');
killPeerCdpScripts('wsk-bootstrap');
...
if (
  ...
  /우숭커우|吴淞口|WSK|邮轮|크루즈/.test(body)
) {
  console.log('【将要】恢复草稿 吴淞口港口（排除吴淞口/火车站）');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /우숭커우|吴淞口|WSK|크루즈|邮轮|上海市区酒店-吴淞口港口/ })
    .filter({ hasNotText: /우숭커우역|吴淞口站|Wusongkou Station|상하이역|火车站|浦东|푸동|PVG/ })
    .first();
  if (await card.count()) {
