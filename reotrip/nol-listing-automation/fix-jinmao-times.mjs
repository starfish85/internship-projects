/**
 * Fix JinMao Tower 88F transfer times for all 4 options: 08:00–21:30 / 30min → 28 slots.
 * Open: 修改选项 → 设置时间 or ⋯编辑 → 重复 小时 添加 → 生成 → 保存 → 临时保存 → 下一个
 * VERIFY 时间段 compact line count===28 before next. Never 提交审核.
 */
...
const EXPECTED = 28;
const NAMES = [
  '상하이 시내 호텔 출발 → 진마오 타워 88층 전망대 편도 이동 (5인승 차량)',
  '상하이 시내 호텔 출발 → 진마오 타워 88층 전망대 편도 이동 (7인승 차량)',
  '진마오 타워 88층 전망대 출발 → 상하이 시내 호텔 편도 이동 (5인승 차량)',
  '진마오 타워 88층 전망대 출발 → 상하이 시내 호텔 편도 이동 (7인승 차량)',
];
...
async function readTimesLine() {
  return page.evaluate(() => {
    const t = document.body.innerText;
    const m = t.match(/时间段\s*\n\s*([0-9:·.\s]+)/);
    if (!m) {
      // also try "时间段" then same line
      const m2 = t.match(/时间段[^\n]*\n([^\n]+)/);
      if (!m2) return { count: 0, raw: null };
      const slots = m2[1]
        .split(/[·.\s]+/)
        .map((s) => s.trim())
        .filter((x) => /^\d{2}:\d{2}$/.test(x));
      return { count: slots.length, first: slots[0], last: slots[slots.length - 1], raw: m2[1].slice(0, 120) };
    }
    const slots = m[1]
      .split(/[·.\s]+/)
      .map((s) => s.trim())
      .filter((x) => /^\d{2}:\d{2}$/.test(x));
    return { count: slots.length, first: slots[0], last: slots[slots.length - 1], raw: m[1].slice(0, 120) };
  });
}
...
// ——— main loop ———
for (let i = 0; i < 4; i++) {
  console.log(`\n======== OPTION ${i + 1} / ${NAMES[i].slice(0, 40)} ========`);
  try {
    await openEdit(i);
    // If already good, skip set but still verify
    let verify = await readTimesLine();
    console.log('  before', verify);
    if (!(verify.count === EXPECTED && verify.first === '08:00' && verify.last === '21:30')) {
      const path = await openTimeEditor();
      console.log('  time open', path);
      if (!path) {
        console.log('  FAIL open time');
        results.push({ i, ok: false, reason: 'no time editor', verify });
        await page.keyboard.press('Escape');
        await sleep(500);
        const ok = page.getByRole('button', { name: /^确定$/ });
        if ((await ok.count()) > 0) await ok.last().click();
      ...
      let slots = await setTimes0800to2130();
      if (!slots || slots.count < EXPECTED) {
        console.log('  WARN slots low, retry once', slots);
        const still = await page.evaluate(() =>
          Array.from(document.querySelectorAll('[role=dialog]')).some((d) =>
            /设置时间|开始时间|重复/.test(d.innerText || ''),
          ),
        );
        if (!still) await openTimeEditor();
        slots = await setTimes0800to2130();
      }
      await sleep(800);
      verify = await readTimesLine();
    }
    console.log('  verify form', verify);
    const ok =
      verify.count === EXPECTED && verify.first === '08:00' && verify.last === '21:30';
    if (!ok) {
      console.log('  FAIL verify — will NOT advance as success');
      results.push({ i, ok: false, verify });
      // still temp-save current state so progress isn't lost, then next
    } else {
      results.push({ i, ok: true, verify });
    }
...
const final = await page.evaluate(() => {
  ...
  return {
    url: location.href,
    mods: (t.match(/修改选项/g) || []).length,
    sell: (t.match(/可销售/g) || []).length,
    jinmao: t.includes('진마오'),
  };
});
