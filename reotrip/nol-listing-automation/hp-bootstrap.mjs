...
const PRODUCT_KO = '도쿄 시내 호텔 ↔ 워너브라더스 스튜디오 투어 도쿄(해리포터) 단독 차량 편도 이동 서비스';
const INTERNAL = '东京市区酒店-哈利波特';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 哈利波特 草稿');
killPeerCdpScripts('hp-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /해리포터|哈利波特|Harry\s*Potter/i.test(body)) {
  console.log('【将要】恢复草稿 哈利波特');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /해리포터|哈利波特|Harry\s*Potter|워너브라더스|东京市区酒店-哈利波特/i })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.hp-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.hp-draft-id', import.meta.url), m[1]);
process.exit(0);
