/**
 * Excel(11) R216–219 · 7/10 售价3792/4108 · CTS-星野 航班 · 车图
 * Excel(11) R216–219 · 7/10 售价3792/4108 · CTS-星野 航班 · 车图
 * Excel(11) R216–219 · 7/10 售价3792/4108 · CTS-星野 航班 · 车图
 */
...
const PRODUCT_KO = '신치토세 공항(CTS) ↔ 호시노 리조트 단독 차량 편도 이동 서비스';
const INTERNAL = '新千岁机场(CTS)-星野度假村';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 新千岁机场(CTS)-星野度假村 草稿');
killPeerCdpScripts('cho-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /신치토세.*호시노|新千岁机场\(CTS\)-星野|星野|Hoshino/i.test(body)) {
  console.log('【将要】恢复草稿 新千岁机场(CTS)-星野度假村');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /新千岁机场\(CTS\)-星野度假村|新千岁.*迪士尼|신치토세 공항\(CTS\) ↔ 도쿄 교토/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.cho-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.cho-draft-id', import.meta.url), m[1]);
process.exit(0);
