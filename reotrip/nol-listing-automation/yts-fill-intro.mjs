/**
 * 横滨邮轮港-东京站 介绍：韩文文案 + 썸네일 3 图 → 保存然后
 * 港口产品：船名/승하선，无航班
 */
import fs from 'fs';
...
const DRAFT = process.env.YTS_DRAFT || readFileSync(join(__dir, '.yts-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/yokohama-cruise-tokyo-stn';
const IMAGES = ['yts-1.jpg', 'yts-2.jpg', 'yts-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter(fs.existsSync);
const CITY = '요코하마 크루즈항';
const DEST = '도쿄역';
const HEADLINE = `${CITY} ↔ ${DEST} 편도 전용 차량으로 여유로운 이동을 즐기세요!`;
const HIGHLIGHT = [
  `${CITY}과 ${DEST}을 편안하게 연결하는 단독 차량 이동`,
  '선박 승하선 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${CITY}(요코하마 크루즈항 / Yokohama Cruise Port)과 ${DEST}(도쿄역 / Tokyo Station) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 요코하마 크루즈항(승하선 터미널) 픽업/샌딩 장소, 도쿄역 승하차 위치, 이용 시간, 연락 가능한 휴대전화 번호, 선박명 및 승하선 정보, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 크루즈 이용 등 요코하마 크루즈항과 도쿄역을 오가시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개까지 적재 가능(최대 4인). 10인승 차량은 26인치 이하 수하물 최대 10개(최대 9인). 차량 공간을 고려해 적합한 차형을 선택해 주세요.

2.미팅 장소: 요코하마 크루즈항 픽업/샌딩 장소와 도쿄역 승하차 위치·이용 시간, 선박명 및 승하선 정보를 정확히 입력해 주세요.

3.본 서비스는 ${CITY}과 ${DEST} 간의 이동만을 포함하며, 중간 정차는 포함되지 않습니다.
...
8.선박 티켓, 아동용 카시트, 야간 할증 등 기타 서비스는 포함되어 있지 않습니다.
...
3.예약 시 호텔 주소, 도쿄역 픽업/샌딩 장소, 선박명·승하선 시간, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('yts-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /요코하마|도쿄역|Yokohama|Tokyo/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /요코하마|도쿄역|Yokohama|Tokyo|크루즈|Cruise/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Yokohama Cruise-Tokyo Stn');
...
let g = await page.evaluate(() => {
  ...
  return {
    ...
    red: /必须至少注册 3|必須至少註冊 3|至少註冊 3/.test(document.body.innerText),
  };
});
console.log('【读回】upload gate', g);
if (g.saveDisabled || g.red) {
  await sleep(10000);
