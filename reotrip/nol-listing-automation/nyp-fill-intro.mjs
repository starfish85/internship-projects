/**
 * 成田机场-横滨港 介绍：韩文 + 用户3图（邮轮码头/横滨港/成田）→ 保存然后
 * 机场+港口：航班 + 선박명
 */
import fs from 'fs';
...
const DRAFT = process.env.NYP_DRAFT || readFileSync(join(__dir, '.nyp-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/narita-yokohama-port';
const IMAGES = ['nyp-1.jpg', 'nyp-2.jpg', 'nyp-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '나리타 공항(NRT)';
const B = '요코하마항';
const HEADLINE = `${A} ↔ ${B} 편도 전용 차량으로 여유로운 이동을 즐기세요!`;
const HIGHLIGHT = [
  `${A}과 ${B}을 편안하게 연결하는 단독 차량 이동`,
  '항공편·선박 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${A}(Narita / Tokyo International Airport)와 ${B}(Yokohama Port) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명·도착/출발 시간·터미널, 요코하마항 내 승하차 또는 픽업 장소, 선박명 및 승하선 정보, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

크루즈·페리 이용 및 나리타 공항 출도착 고객께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 항공편·터미널 정보와 요코하마항 픽업/샌딩 장소, 선박명·승하선 시간, 이용 시간을 정확히 입력해 주세요.
...
3.예약 시 항공편명, 도착 또는 출발 시간, 터미널, 요코하마항 픽업/샌딩 장소, 선박명·승하선 정보, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('nyp-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /나리타|NRT|요코하마/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /나리타|Narita|요코하마|Yokohama/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Narita-Yokohama');
