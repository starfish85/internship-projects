/**
 * 北京市区酒店 ↔ 北京大兴机场(PKX) 私人接送
 * Excel: ~/Downloads/NOL 待上架产品 (6).xlsx → 国内接送产品 R50–51
 * 仅 7 座 × 去/返（无 5 座）：
 *   7 去 成本 325.5 → 平日 407 / 五一·十一 581 / 春节 814  (D=C/0.8, E=D/0.7, F=D/0.5 半入 round)
 *   7 返 成本 406.88 → 平日 509 / 五一·十一 727 / 春节 1017  （去≠返，禁止对称抄写）
...
const DEST_KO = '베이징 다싱국제공항(PKX)';
const DEST_KO_SHORT = '베이징 다싱국제공항(PKX)';
const CITY_KO = '베이징 시내 호텔';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/beijing-daxing-airport';
const IMAGES = ['pkx-1.jpg', 'pkx-2.jpg', 'pkx-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

// Excel 售价列公式求值整数（round half up）
const PRICES = {
  '7go': { normal: '407', oct: '581', spring: '814', may: '581' },
  '7rtn': { normal: '509', oct: '727', spring: '1017', may: '727' },
};
...
const INTRO = `이 서비스는 ${CITY_KO}과 ${DEST_KO}(北京大兴国际机场 / Beijing Daxing International Airport) 사이의 편도 전용 차량 이동 서비스입니다.
...
가족, 출장, 소규모 그룹 등 베이징 다싱국제공항을 이용하시는 분들께 적합합니다.`;
...
async function getPage() {
  ...
  return connectNolPage({ selfHint: 'list-beijing-daxing', killPeers: true });
}
...
async function bootstrap(page) {
  ...
  if (
    ...
    const card = page
      ...
      .filter({ hasText: /다싱국제공항|大兴机场|PKX|베이징 시내 호텔 ↔ 베이징 다싱/ })
      .first();
...
async function fillAttrs(page) {
  ...
  // POI 北京大兴国际机场 PKX — 文字/元素定位，禁止坐标
  const hasPoi = await page.evaluate(() =>
    /大兴|Daxing|PKX|다싱국제공항|大兴国际/i.test(document.body.innerText),
  );
  if (!hasPoi) {
    ...
    await search.fill('北京大兴国际机场');
    await page.keyboard.press('Enter');
    await sleep(2200);
    // 元素点击结果行：含大兴/Daxing，排除首都
    let clicked = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('div,li,button')).filter((el) => {
        ...
        return (
          ...
          r.height < 180 &&
          r.y > 80 &&
          /大兴|Daxing|PKX/i.test(t) &&
          /北京|Beijing|中国|中华|机场|Airport/i.test(t) &&
          !/首都|Capital(?!.*Daxing)|서울|上海|浦东|虹桥/.test(t)
        );
      });
    ...
    if (!clicked) {
      await search.fill('Beijing Daxing International Airport');
      await page.keyboard.press('Enter');
      await sleep(2200);
      clicked = await page.evaluate(() => {
        const cards = Array.from(document.querySelectorAll('div,li,button')).filter((el) => {
          ...
          return (
            ...
            r.height > 40 &&
            r.height < 180 &&
            /Daxing|大兴|PKX/i.test(t) &&
            /Beijing|北京|Airport|机场/i.test(t) &&
            !/Capital|首都/.test(t)
          );
        });
        if (!cards[0]) return null;
