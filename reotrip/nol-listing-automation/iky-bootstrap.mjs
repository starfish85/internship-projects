/**
 * 伊丹机场(ITM)-京都市区 — 创建或恢复草稿
 * Excel(11) 日本接送 R192–195 · 7/10 售价1580/1896 · 机场+市区 航班
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '이타미 공항(ITM) ↔ 교토 시내 단독 차량 편도 이동 서비스';
const INTERNAL = '伊丹机场(ITM)-京都市区';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 伊丹机场(ITM)-京都市区 草稿');
killPeerCdpScripts('iky-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /이타미.*교토|伊丹机场\(ITM\)-京都市区|伊丹.*京都|ITM.*Kyoto/i.test(body)) {
  console.log('【将要】恢复草稿 伊丹机场(ITM)-京都市区');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /伊丹机场\(ITM\)-京都市区|伊丹.*迪士尼|이타미 공항\(ITM\) ↔ 도쿄 교토/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.iky-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.iky-draft-id', import.meta.url), m[1]);
process.exit(0);
