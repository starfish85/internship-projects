...
const PRODUCT_KO = '도쿄 시내 호텔 ↔ 도쿄 스카이트리 단독 차량 편도 이동 서비스';
const INTERNAL = '东京市区酒店-晴空塔';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 晴空塔 草稿');
killPeerCdpScripts('st-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /스카이트리|晴空塔|Skytree|スカイツリー/i.test(body)) {
  console.log('【将要】恢复草稿 晴空塔');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /스카이트리|晴空塔|Skytree|スカイツリー|东京市区酒店-晴空塔/i })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.st-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.st-draft-id', import.meta.url), m[1]);
process.exit(0);
