 ...
 */
/**
 * 上海市区酒店 ↔ Top of Shanghai Observatory 私人接送
 * Excel 售价（公式求值，勿另算）：
 * 5: 213 / 304 / 425
 * 7 go: 250 / 357 / 510
 * 7 rtn: 250 / 357 / 500
...
const PRODUCT_KO =
  '상하이 시내 호텔 ↔ 탑 오브 상하이 전망대 단독 차량 편도 이동 서비스';
const INTERNAL = '上海市区酒店-Top of Shanghai Observatory Ticket';
const DEST_KO = '탑 오브 상하이 전망대';
const DEST_KO_SHORT = '탑 오브 상하이';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/top-of-shanghai';
const IMAGES = ['top-shanghai-1.jpg', 'top-shanghai-2.jpg', 'top-shanghai-3.jpg']
...
const INTRO = `이 서비스는 상하이 시내 호텔과 ${DEST_KO}(Top of Shanghai Observatory) 사이의 편도 전용 차량 이동 서비스입니다.
...
가족, 커플, 소규모 그룹 등 상하이 타워 전망대를 방문하시는 분들께 적합합니다.
...
async function bootstrap(page) {
  ...
  if (body.includes('탑 오브 상하이') || body.includes('Top of Shanghai')) {
    console.log('Resume existing draft');
    const card = page.locator('div[class*="slot___StyledContainer4"]').filter({ hasText: /탑 오브 상하이|Top of Shanghai/ }).first();
...
async function fillAttrs(page) {
  ...
  const hasPoi = await page.evaluate(() =>
    /上海中心|Shanghai Tower|탑 오브|观景|전망|浦东|Lujiazui|世纪大道/i.test(document.body.innerText),
  );
  if (!hasPoi) {
    await page.evaluate(() => {
      Array.from(document.querySelectorAll('button'))
    ...
    const picked = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('div')).filter((el) => {
        ...
        return (
          ...
          r.y > 100 &&
          r.y < 850 &&
          /上海中心|Shanghai Tower|Top of Shanghai|观景|世纪大道|浦东|Lujiazui|中国|중华|中华/i.test(t) &&
          !/서울|대한민국/.test(t)
        );
      });
...
async function main() {
  ...
  if (id) fs.writeFileSync('/tmp/nol-top-shanghai-id.txt', id);
