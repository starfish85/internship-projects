...
const DRAFT = process.env.HN_DRAFT || readFileSync(join(__dir, '.hn-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/hakone-narita';
const IMAGES = ['hn-1.jpg', 'hn-2.jpg', 'hn-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '하코네 시내 호텔';
const B = '나리타 공항(NRT)';
...
const INTRO = `이 서비스는 ${A}(Hakone)와 ${B}(Narita / Tokyo International Airport) 사이의 편도 전용 차량 이동 서비스입니다.
...
하코네 온천·관광 후 나리타 공항으로 이동하시거나, 나리타 도착 후 하코네로 이동하시는 분들께 적합합니다.`;
...
const textOk = await page.evaluate(() => ({
  hl: /하코네|나리타|NRT|Hakone|Narita/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /하코네|나리타|Hakone|Narita|NRT/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Hakone-Narita');
