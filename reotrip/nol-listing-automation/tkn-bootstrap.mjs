/**
* 东京-长野 — 创建或恢复草稿
 * Excel(11) 日本接送 R184–187 · 7/10 成本→售价5266/5602 · 城际无航班 · 无假日
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '도쿄 ↔ 나가노 단독 차량 편도 이동 서비스';
const INTERNAL = '东京-长野';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 东京-长野 草稿');
killPeerCdpScripts('tkn-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /도쿄.*나가노|东京-长野|東京-長野|东京.*长野/i.test(body)) {
  console.log('【将要】恢复草稿 东京-长野');
  const card = page
  ...
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.tkn-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.tkn-draft-id', import.meta.url), m[1]);
process.exit(0);
