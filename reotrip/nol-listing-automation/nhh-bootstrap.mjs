/**
 * 成田机场(NRT)-羽田机场(HND) — 创建或恢复草稿
 * Excel(11) R172–175 · 7/10 成本→售价1255/1961 · 机场航班预约 日本接送 Excel(10) R107–110 · 7/10 售价1580/2054 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '나리타 공항(NRT) ↔ 하네다 공항(HND) 단독 차량 편도 이동 서비스';
const INTERNAL = '成田机场(NRT)-羽田机场(HND)';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 成田机场(NRT)-羽田机场(HND) 草稿');
killPeerCdpScripts('nhh-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /나리타.*하네다|成田机场\(NRT\)-羽田|成田.*羽田/i.test(body)) {
  console.log('【将要】恢复草稿 成田机场(NRT)-羽田机场(HND)');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /成田机场\(NRT\)-羽田|成田.*羽田|나리타 공항\(NRT\) ↔ 하네다/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.nhh-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.nhh-draft-id', import.meta.url), m[1]);
process.exit(0);
