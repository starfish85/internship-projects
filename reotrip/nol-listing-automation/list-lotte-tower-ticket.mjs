/**
 * 乐天塔 Lotte World Tower Observation Deck
 * CSV 韩国门票产品 R2: 一日票 · 只卖成人 · 外籍特惠（不支持韩国国籍）
 * 【售价来源】售价hkd=156（表显示；加价逻辑 124.5 未用；未自算）skill 0h
 * 单选项 1일 입장권；0n 双门禁 代表+按数量
 * 串行：本脚本只上乐天塔，禁并行乐天世界/其他
 * Never 提交审核
 *
 * usage: node list-lotte-tower-ticket.mjs [bootstrap|attrs|intro|regs|options]
 */
...
const DRAFT_FILE = join(__dir, '.lotte-tower-draft-id');
const PHASE = process.argv[2] || 'bootstrap';
// 【售价来源】售价hkd=252（CSV 韩国门票 R2 表显示；加价逻辑=124.5 未用；未自算）
const PRICE = (process.env.LOTTE_TOWER_PRICE || process.argv[3] || '156').trim();
const PRODUCT_KO = '롯데월드타워 전망대 1일 입장권 (외국인 특혜)';
const INTERNAL = '乐天塔 Lotte World Tower 1日票 外籍特惠';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/lotte-tower';
const IMAGES = ['lt-1.jpg', 'lt-2.jpg', 'lt-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '서울 롯데월드타워 전망대에서 스카이라인을 감상하세요!';
const HIGHLIGHT = [
  '롯데월드타워 전망대 1일 입장권 · 성인 전용 · 외국인 특혜 요금',
  '서울 최고층 전망 · 이메일 전자티켓/이용 안내',
  '한국 국적자 구매 불가 · 예약 시 전화·이메일·여권 정보 필수',
].join('\n');
const INTRO = `롯데월드타워(Lotte World Tower) 전망대 1일 입장권입니다.
서울 잠실의 랜드마크 타워 전망대에서 도심과 한강 일대를 조망할 수 있습니다.

포함 사항:
- 롯데월드타워 전망대 1일 입장권 1매 (성인)
...
서울 스카이라인 전망을 즐기고 싶은 외국인 이용객께 추천합니다.`;
...
function readDraft() {
  if (process.env.LOTTE_TOWER_DRAFT) return process.env.LOTTE_TOWER_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=124.5 未用作售价；未自算）');
  ...
  // resume 必须 롯데월드타워/Lotte Tower；禁误点 롯데월드 乐园
  if (/롯데월드타워|롯데타워|Lotte World Tower|Lotte Tower|乐天塔/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /롯데월드타워|롯데타워|Lotte World Tower|Lotte Tower|乐天塔|전망대/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /乐天塔|樂天塔|Lotte Tower|롯데월드타워|롯데타워|World Tower|잠실|Jamsil/.test(t)
...
async function attrs(page) {
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 롯데월드타워 / Lotte World Tower Seoul');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['Lotte World Tower', '롯데월드타워', '롯데타워', 'Lotte Tower Seoul', '乐天塔'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const score = (s) => {
            if (/Hotel|호텔|酒店|숙박|住宿/i.test(s)) return -50;
            if (/广州|廣州|北京|上海|Macau|澳门/i.test(s)) return -50;
            if (/롯데월드타워|Lotte World Tower|롯데타워|Lotte Tower/i.test(s) && /서울|Seoul|잠실|Jamsil|Songpa|韩国|Korea/i.test(s))
              return 100;
            if (/롯데월드타워|Lotte World Tower|롯데타워|Lotte Tower/i.test(s)) return 80;
            return 0;
          };
          let best = null;
          let bs = 0;
...
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '롯데월드타워 전망대 1일 입장권' }],
...
async function options(page) {
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('롯데월드타워 전망대 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
