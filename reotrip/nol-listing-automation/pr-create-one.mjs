/**
 * 上海浦东机场 ↔ 上海火车站 创建单个选项（无时段，时段另批）
 * usage: node pr-create-one.mjs go|rtn
 */
...
const AIR = '상하이 푸동국제공항(PVG)';
const STN = '상하이역';
const isRtn = process.argv[2] === 'rtn';
const OPT = isRtn
  ? {
      key: '7rtn',
      name: `${STN} 출발 → ${AIR} 편도 이동 (7인승 차량)`,
      desc: `${STN} 출발 → ${AIR} 편도 이동 (7인승 차량, 최대 4인 탑승 가능)\n24인치 이하 수하물 기준: 3~5개 적재 가능\n별도 항공권·기차표, 아동용 카시트, 야간 할증은 포함되어 있지 않습니다.`,
      pt: '7인승 오는',
      ptd: '7인승 차량',
      price: '350',
    }
  : {
      key: '7go',
      name: `${AIR} 출발 → ${STN} 편도 이동 (7인승 차량)`,
      desc: `${AIR} 출발 → ${STN} 편도 이동 (7인승 차량, 최대 4인 탑승 가능)\n24인치 이하 수하물 기준: 3~5개 적재 가능\n별도 항공권·기차표, 아동용 카시트, 야간 할증은 포함되어 있지 않습니다.`,
      pt: '7인승 가는',
      ptd: '7인승 차량',
      price: '350',
    };
...
killPeerCdpScripts('pr-create-one');
