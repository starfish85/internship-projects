/**
 * 新千岁机场(CTS)-登别 — 创建或恢复草稿
 * Excel (9) 日本接送 Excel(10) R107–110 · 7/10 售价1580/2054 · 无假日
 * 机场+港口：航班预约 + 船名文案
 */
...
const PRODUCT_KO = '신치토세 공항(CTS) ↔ 노보리베츠 단독 차량 편도 이동 서비스';
const INTERNAL = '新千岁机场(CTS)-登别';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 新千岁机场(CTS)-登别 草稿');
killPeerCdpScripts('cn-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /신치토세.*노보리베츠|新千岁机场\(CTS\)-登别|新千岁机场.*登别/i.test(body)) {
  console.log('【将要】恢复草稿 新千岁机场(CTS)-登别');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /新千岁机场\(CTS\)-登别|新千岁机场.*登别|신치토세 공항\(CTS\) ↔ 노보리베츠/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.cn-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.cn-draft-id', import.meta.url), m[1]);
process.exit(0);
