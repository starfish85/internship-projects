/**
 * 名古屋市区-高山 — 创建或恢复草稿
 * Excel(11) 日本接送 R124–127 · 7/10 售价3668/4006 · 无假日 · 城际无航班
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '나고야 시내 ↔ 다카야마 단독 차량 편도 이동 서비스';
const INTERNAL = '名古屋市区-高山';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 名古屋市区-高山 草稿');
killPeerCdpScripts('ngt-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /나고야 시내.*다카야마|名古屋市区-高山/i.test(body)) {
  console.log('【将要】恢复草稿 名古屋市区-高山');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /名古屋市区-高山|나고야 시내 ↔ 다카야마/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.ngt-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.ngt-draft-id', import.meta.url), m[1]);
process.exit(0);
