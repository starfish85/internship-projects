/**
 * 上海虹桥机场(SHA) — 创建或恢复草稿
 * Excel R56–57 · 内部名 上海市区酒店-上海虹桥机场
 * §55 单步 · §52 视口 · §53 杀残留
 */
...
const PRODUCT_KO =
  '상하이 시내 호텔 ↔ 상하이 훙차오국제공항(SHA) 단독 차량 편도 이동 서비스';
const INTERNAL = '上海市区酒店-上海虹桥机场';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 SHA 草稿');
killPeerCdpScripts('sha-bootstrap');
...
if (
  ...
  /훙차오국제공항|虹桥机场|SHA/.test(body)
) {
  console.log('【将要】恢复草稿 虹桥机场（排除虹桥/火车站）');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /훙차오국제공항|虹桥机场|SHA|上海市区酒店-上海虹桥机场/ })
    .filter({ hasNotText: /훙차오역|虹桥站|Hongqiao Station|상하이역|火车站|浦东|푸동|PVG/ })
    .first();
  if (await card.count()) {
