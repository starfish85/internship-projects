/**
 * Canton Tower 450M Look Out Ticket
 * Excel/CSV 国内景区门票 R31: 450M 观景 · 只卖成人
 * 售价：只贴表格「售价hkd」列显示值；禁止用加价逻辑 255.82 自算（skill 0h）
 * Family A style: adult only, 1 option, email ticket, passport+name
 * Never 提交审核
 *
 * usage: node list-canton-tower-450-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 */
...
const DRAFT_FILE = join(__dir, '.canton-tower-450-draft-id');
const PHASE = process.argv[2] || 'all';
// 硬规则 0h：只允许表内显示售价，禁止自算 467/0.8
// 【售价来源】售价hkd=320（CSV R31 表显示；加价逻辑=255.82 未用；未自算）
const PRICE = (process.env.CANTON_TOWER_450_PRICE || process.argv[3] || '320').trim();
const PRODUCT_KO = '광저우 캔톤타워 450M 전망대 1일 입장권';
const INTERNAL = 'Canton Tower 450M Look Out Ticket';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/canton-tower-450';
const IMAGES = ['tower-1.jpg', 'tower-2.jpg', 'tower-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '광저우 캔톤타워 450M 전망대에서 시티 스카이라인을 감상하세요!';
const HIGHLIGHT = [
  '캔톤타워 450M 전망대 1일 입장권 · 성인 전용',
  '450M 높이에서 광저우 시내·주강 조망',
  '이메일로 이용 안내/전자티켓 수령',
].join('\n');
const INTRO = `광저우 캔톤타워(Canton Tower) 450M 전망대 1일 입장권입니다.
450미터 높이 전망대에서 광저우 도심과 주강 일대를 조망할 수 있습니다.

포함 사항:
- 캔톤타워 450M 전망대 1일 입장권 1매 (성인)
...
광저우의 랜드마크 전망을 즐기고 싶은 분들께 추천합니다.`;
...
function readDraft() {
  if (process.env.CANTON_TOWER_450_DRAFT) return process.env.CANTON_TOWER_450_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=255.82 未用作售价；未自算）');
  ...
  if (/Canton Tower|캔톤타워|广州塔|廣州塔|450M|Canton Tower 450/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /Canton Tower|캔톤타워|广州塔|廣州塔|450M/i })
      .first();
...
async function options(page) {
  if (!PRICE) {
    ...
    console.log('请设置: CANTON_TOWER_450_PRICE=<表显示> node list-canton-tower-450-ticket.mjs options');
    process.exit(2);
  }
  const id = readDraft();
  if (!id) throw new Error('no draft');
