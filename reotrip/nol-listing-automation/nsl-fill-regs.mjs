/**
 * 名古屋-铃鹿赛道/乐高乐园 法规：截单/取消/包含/预约(无航班)/凭证 → 保存然后
 * §51 预约 label + 摘要非空
 */
...
const CITY_KO = '나고야';
const DEST_KO = '스즈카 서킷/레고랜드 재팬';
const INCLUDE_TR = `${CITY_KO} ↔ ${DEST_KO} 편도 전용 차량 이동 및 주차비 포함`;
const INCLUDE_PU = '픽업/샌딩 서비스 및 주차비 포함';
const EXCLUDE =
  '입장권·기타 티켓, 가이드, 팁, 개인 비용, 아동용 카시트, 야간 할증, 추가 경유지 및 기타 명시되지 않은 비용은 포함되어 있지 않습니다.';
const RESV_IDS = [
  'CELLPHONE-required',
  'EMAIL-required',
...
killPeerCdpScripts('nsl-fill-regs');
