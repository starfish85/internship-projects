/**
 * 北京市区酒店 ↔ 北京首都机场(PEK) 私人接送
 * Excel: ~/Downloads/NOL 待上架产品 (6).xlsx → 国内接送产品 R48–49
 * 仅 7 座 × 去/返（无 5 座）：
 *   7 去 成本 174.38 → 平日 218 / 五一·十一 311 / 春节 436  (D=C/0.8, E=D/0.7, F=D/0.5 半入 round)
 *   7 返 成本 255.75 → 平日 320 / 五一·十一 457 / 春节 639  （去≠返，禁止对称抄写）
 * 时段 08:00–21:30 ×28；机场预约含航班；停选项列表；永不提交审核
 * 时段唯一：lib/set-times-china.mjs
 */
...
const PRODUCT_KO =
  '베이징 시내 호텔 ↔ 베이징 수도국제공항(PEK) 단독 차량 편도 이동 서비스';
const INTERNAL = '北京市区酒店-北京首都机场';
const DEST_KO = '베이징 수도국제공항(PEK)';
const DEST_KO_SHORT = '베이징 수도국제공항(PEK)';
const CITY_KO = '베이징 시내 호텔';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/beijing-capital-airport';
const IMAGES = ['pek-1.jpg', 'pek-2.jpg', 'pek-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

// Excel 售价列公式求值整数（round half up）
const PRICES = {
  '7go': { normal: '218', oct: '311', spring: '436', may: '311' },
  '7rtn': { normal: '320', oct: '457', spring: '639', may: '457' },
};
...
const HIGHLIGHT = [
  ...
  '7인승 차량 · 최대 4인 + 수하물 4개(24인치 이하)',
].join('\n');
const INTRO = `이 서비스는 ${CITY_KO}과 ${DEST_KO}(北京首都国际机场 / Beijing Capital International Airport) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명, 도착/출발 시간, 베이징 시내 호텔명/주소, 픽업 장소, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

가족, 출장, 소규모 그룹 등 베이징 수도국제공항을 이용하시는 분들께 적합합니다.`;

const MUST_KNOW = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 4개까지 적재 가능. 차량 공간을 고려해 적합한 차형을 선택해 주세요.

2.미팅 장소: 호텔 픽업 시 기사님이 호텔 로비에서 픽업해 드립니다. 동일한 이름의 호텔이 있을 수 있으므로 호텔 이름과 주소를 함께 제공해 주세요. 공항 픽업 시 터미널/출구 정보를 정확히 입력해 주세요.
...
3.예약 시 항공편명, 도착 또는 출발 시간, 터미널, 호텔 정보, 인원 및 수하물을 정확히 입력해 주세요.`;
...
async function bootstrap(page) {
  ...
  if (
    ...
    const card = page
      ...
      .filter({ hasText: /수도국제공항|首都机场|PEK|베이징 시내 호텔 ↔ 베이징 수도/ })
      .first();
...
async function fillAttrs(page) {
  ...
  const hasPoi = await page.evaluate(() =>
    /首都|Capital|PEK|수도공항|수도국제공항|首都国际/i.test(document.body.innerText),
  );
  if (!hasPoi) {
    await page.evaluate(() => {
      Array.from(document.querySelectorAll('button'))
    ...
    let picked = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('div')).filter((el) => {
        ...
        return (
          ...
          r.y > 100 &&
          r.y < 850 &&
          /首都|Capital|PEK|国际机场|国际機場|Airport/i.test(t) &&
          /北京|Beijing|中国|中华/i.test(t) &&
          !/大兴|Daxing|PKX|서울|上海|浦东|虹桥/.test(t)
        );
      });
      if (!cards[0]) return null;
      const r = cards[0].getBoundingClientRect();
      return { t: cards[0].innerText.slice(0, 100), x: r.x + 40, y: r.y + 20 };
    });
    if (!picked) {
      await search.fill('Beijing Capital International Airport');
      await page.keyboard.press('Enter');
      await sleep(2000);
      picked = await page.evaluate(() => {
        const cards = Array.from(document.querySelectorAll('div')).filter((el) => {
          ...
          return (
            ...
            r.y > 100 &&
            r.y < 850 &&
            /Capital|首都|PEK/i.test(t) &&
            /Beijing|北京/i.test(t) &&
            !/Daxing|大兴|PKX/.test(t)
          );
        });
        if (!cards[0]) return null;
        const r = cards[0].getBoundingClientRect();
    ...
    console.log('POI pick', picked?.t || picked);
    if (picked?.t) {
      // §43: 文案点 POI 结果，禁止坐标
      const row = page.locator('div,li,button').filter({ hasText: picked.t.slice(0, 24) }).first();
      await row.click({ timeout: 10000 }).catch(async () => {
        await page.getByText(/Capital|首都|PEK/).first().click({ timeout: 8000 });
      });
      await sleep(1000);
    }
    await page.evaluate(() => {
...
async function holidays(page) {
  ...
  for (let oi = 0; oi < OPTIONS.length; oi++) {
    ...
    for (const seg of segs) {
      ...
      if (await loc.isDisabled()) throw new Error(`price disabled oi=${oi}`);
...
async function main() {
  ...
  const id = new URL(page.url()).searchParams.get('id');
  if (id) fs.writeFileSync('/tmp/nol-beijing-capital-id.txt', id);
