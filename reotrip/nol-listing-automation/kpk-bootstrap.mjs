/**
 * 神户港-京都市区 — 创建或恢复草稿
 * Excel(11) R228–231 · 7/10 售价1738/2020 · 神户港-京都 港口선박 · 车图
 */
import { chromium } from 'playwright';
import { writeFileSync } from 'node:fs';
import { killPeerCdpScripts, failExit } from './lib/cdp-session.mjs';

const PRODUCT_KO = '고베항 ↔ 교토 시내 단독 차량 편도 이동 서비스';
const INTERNAL = '神户港-京都市区';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

console.log('【将要】杀残留 CDP + 打开列表 创建/恢复 神户港-京都市区 草稿');
killPeerCdpScripts('kpk-bootstrap');
...
if (body.includes(INTERNAL) || body.includes(PRODUCT_KO) || /고베항.*교토|神户港-京都市区|神户港.*京都|Kobe.*Kyoto/i.test(body)) {
  console.log('【将要】恢复草稿 神户港-京都市区');
  const card = page
    .locator('div[class*="slot___StyledContainer4"]')
    .filter({ hasText: /神户港-京都市区|고베항 ↔ 교토 시내/ })
    .first();
  if (await card.count()) {
    ...
    if (m?.[1]) writeFileSync(new URL('./.kpk-draft-id', import.meta.url), m[1]);
    process.exit(0);
  }
}
...
writeFileSync(new URL('./.kpk-draft-id', import.meta.url), m[1]);
process.exit(0);
