/**
 * 中部国际机场(NGO)-名古屋市区酒店 介绍 + 用户3图 → 保存然后
 * 机场产品：航班预约
 */
import fs from 'fs';
...
const DRAFT = process.env.CNH_DRAFT || readFileSync(join(__dir, '.cnh-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/centrair-nagoya';
const IMAGES = ['cnh-1.jpg', 'cnh-2.jpg', 'cnh-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '중부 국제공항(NGO)';
const B = '나고야 시내 호텔';
...
const INTRO = `이 서비스는 ${A}(Chubu Centrair International Airport)와 ${B}(나고야 시내 호텔 / Nagoya City Hotel area) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명·도착/출발 시간·터미널, 나고야 시내 호텔명·주소 및 픽업/샌딩 장소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

중부 국제공항(Centrair) 출도착 후 나고야 시내 호텔로 이동하시거나, 시내 호텔에서 공항으로 이동하시는 분들께 적합합니다.`;
const MUST = `1.수하물 규격: 24인치 이하 수하물 기준, 7인승 차량 최대 5개(최대 4인). 10인승 차량 26인치 이하 최대 10개(최대 9인). 적합한 차형을 선택해 주세요.

2.미팅 장소: 항공편·터미널 정보와 나고야 시내 호텔/픽업 장소, 이용 시간을 정확히 입력해 주세요.
...
3.예약 시 항공편명, 도착 또는 출발 시간, 터미널, 나고야 시내 호텔 주소, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('cnh-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /중부|나고야|NGO|Centrair|Nagoya/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /중부|나고야|NGO|Centrair|Nagoya/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing NGO-Nagoya');
