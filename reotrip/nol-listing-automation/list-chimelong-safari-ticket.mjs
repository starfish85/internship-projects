/**
 * Guangzhou Chimelong Safari Park Admission Ticket
 * Excel 国内景区门票 R28: 一日票 · 只卖成人
 * 售价：只贴表格「售价hkd」列显示值；禁止用加价逻辑 406.99 自算（skill 0h）
 * Family A style: adult only, 1 option, email ticket, passport+name
 * Never 提交审核
 *
 * usage: node list-chimelong-safari-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 * PRICE: env CHIMELONG_PRICE=表显示售价 或 argv 第3参数
 */
...
const DRAFT_FILE = join(__dir, '.chimelong-safari-draft-id');
const PHASE = process.argv[2] || 'all';
// 硬规则 0h：只允许表内显示售价，禁止自算 406.99/0.8
// 【售价来源】售价hkd=509（CSV 国内景区门票 R28 表显示；加价逻辑=406.99 未用；未自算）
const PRICE = (process.env.CHIMELONG_PRICE || process.argv[3] || '509').trim();
const PRODUCT_KO = '광저우 침롱 야생동물세계 1일 입장권';
const INTERNAL = 'Guangzhou Chimelong Safari Park Admission Ticket';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/chimelong-safari';
const IMAGES = ['safari-1.jpg', 'safari-2.jpg', 'safari-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '광저우 침롱 야생동물세계에서 하루 종일 동물과 자연을 만나보세요!';
const HIGHLIGHT = [
  '침롱 야생동물세계 1일 입장권 · 성인 전용',
  '코알라·플라밍고 등 인기 동물 관람',
  '이메일로 이용 안내/전자티켓 수령',
].join('\n');
const INTRO = `광저우 침롱 야생동물세계(Guangzhou Chimelong Safari Park) 1일 입장권입니다.
하루 동안 파크 내 동물 관람과 테마 존을 자유롭게 이용할 수 있습니다.

포함 사항:
- 침롱 야생동물세계 1일 입장권 1매 (성인)
...
function readDraft() {
  if (process.env.CHIMELONG_DRAFT) return process.env.CHIMELONG_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=406.99 未用作售价；未自算）');
  ...
  if (/Chimelong Safari|침롱 야생동물|长隆野生动物/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /Chimelong Safari|침롱 야생동물|长隆野生动物/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /广州|廣州|Guangzhou|长隆|長隆|Chimelong|番禺|野生动物|Safari|公园|公園/.test(t)
...
async function attrs(page) {
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 广州 长隆 / Chimelong Safari');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['长隆野生动物世界', 'Chimelong Safari Park', '广州长隆', 'Panyu Chimelong'];
      let clicked = null;
          ...
          const hit =
            items.find((e) => {
              ...
              if (/한국|Korea|首尔|서울|호텔|Hotel|酒店/i.test(s) && !/广州|廣州|Guangzhou|长隆|長隆|Chimelong|番禺|中国|中國/i.test(s))
                return false;
              return /广州|廣州|Guangzhou|长隆|長隆|Chimelong|番禺|野生动物|Safari|中国|中國/i.test(s);
            }) || null;
          if (hit) {
            hit.click();
...
async function toggleResvIds(page, ids) {
  ...
  for (const rid of ids) {
    const lab = page.locator(`label[for="${rid}"]`);
    const inp = page.locator(`#${CSS.escape ? rid : rid}`);
    if ((await lab.count()) > 0) {
      await lab.first().scrollIntoViewIfNeeded().catch(() => {});
      const before = (await page.locator(`#${rid}`).count())
        ? await page.locator(`#${rid}`).isChecked().catch(() => false)
        : false;
      if (!before) {
        await lab.first().click({ force: true });
        await sleep(150);
      }
      results[rid] = await page.locator(`#${rid}`).isChecked().catch(() => false);
    } else {
...
async function regs(page) {
  ...
  try {
    ...
    if ((await writeInc.count()) > 0) {
      ...
      if ((await desc.count()) > 0) await desc.fill('침롱 야생동물세계 1일 입장권').catch(() => {});
      await page.evaluate(() => {
...
async function tempThenNext(page, label) {
  ...
  const pick = temps.filter((t) => !t.d).sort((a, b) => a.w - b.w)[0];
  if (!pick) {
    console.log(label, 'temp MISSING', temps);
    return false;
  }
  await page.locator('button').nth(pick.i).click();
  console.log(label, 'temp', pick);
  await sleep(2500);
  for (let t = 0; t < 20; t++) {
...
async function options(page) {
  if (!PRICE) {
    console.log('【结果】FAIL 售价hkd 表显示未提供 — 硬规则0h 禁止自算 406.99/0.8');
    console.log('请设置: CHIMELONG_PRICE=<表显示> node list-chimelong-safari-ticket.mjs options');
    process.exit(2);
  }
  const id = readDraft();
  if (!id) throw new Error('no draft');
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('침롱 야생동물세계 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
  ...
  const final = await page.evaluate(() => {
    ...
    return {
      ...
      priceHit: PRICE ? (t.match(new RegExp(PRICE.replace('.', '\\.'), 'g')) || []).length : 0,
      snip: t.slice(0, 700),
    };
  });
...
console.log('【复用入口】门票 skill P0–P10 + 售价列 0h');
console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=406.99 未用作售价；未自算）');
