/**
 * Guangzhou Chimelong International Circus Admission Ticket
 * Excel/CSV 国内景区门票 R30: 一日票 · 只卖成人
 * 售价：只贴表格「售价hkd」列显示值；禁止用加价逻辑 467 自算（skill 0h）
 * Family A style: adult only, 1 option, email ticket, passport+name
 * Never 提交审核
 *
 * usage: node list-chimelong-circus-ticket.mjs [bootstrap|attrs|intro|regs|options|all]
 */
...
const DRAFT_FILE = join(__dir, '.chimelong-circus-draft-id');
const PHASE = process.argv[2] || 'all';
// 硬规则 0h：只允许表内显示售价，禁止自算 467/0.8
// 【售价来源】售价hkd=584（CSV R30 表显示；加价逻辑=467 未用；未自算）
const PRICE = (process.env.CHIMELONG_CIRCUS_PRICE || process.argv[3] || '584').trim();
const PRODUCT_KO = '광저우 침롱 국제서커스 1일 입장권';
const INTERNAL = 'Guangzhou Chimelong International Circus Admission Ticket';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/chimelong-circus';
const IMAGES = ['circus-1.jpg', 'circus-2.jpg', 'circus-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '광저우 침롱 국제서커스에서 세계급 서커스를 즐겨보세요!';
const HIGHLIGHT = [
  '침롱 국제서커스 1일 입장권 · 성인 전용',
  '세계급 서커스·클라운·동물 공연 관람',
  '이메일로 이용 안내/전자티켓 수령',
].join('\n');
const INTRO = `광저우 침롱 국제서커스(Guangzhou Chimelong International Circus) 1일 입장권입니다.
세계적인 서커스 공연과 클라운·동물 쇼를 관람할 수 있는 입장권입니다.

포함 사항:
- 침롱 국제서커스 1일 입장권 1매 (성인)
...
4. 공연 시간·좌석·입장 규정은 이메일 전자티켓/이용 안내 및 현장 공지를 따릅니다.`;
...
function readDraft() {
  if (process.env.CHIMELONG_CIRCUS_DRAFT) return process.env.CHIMELONG_CIRCUS_DRAFT.trim();
...
async function bootstrap(page) {
  console.log('【售价来源】售价hkd=' + (PRICE || '(待表显示)') + '（加价逻辑=467 未用作售价；未自算）');
  ...
  if (/Chimelong International Circus|침롱 국제서커스|长隆国际大马戏|長隆國際大馬戲|Chimelong Circus/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /Chimelong International Circus|침롱 국제서커스|长隆国际大马戏|長隆國際大馬戲|Chimelong Circus/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /广州|廣州|Guangzhou|长隆|長隆|Chimelong|番禺|马戏|馬戲|Circus|서커스|国际大马戏|國際大馬戲|公园|公園/.test(t)
...
async function attrs(page) {
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 广州 长隆国际大马戏 / Chimelong International Circus');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['长隆国际大马戏', 'Chimelong International Circus', '广州长隆马戏', 'Guangzhou Chimelong Circus'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const hit =
            items.find((e) => {
              const s = e.innerText || '';
              if (/한국|Korea|首尔|서울|호텔|Hotel|酒店/i.test(s) && !/广州|廣州|Guangzhou|长隆|長隆|Chimelong|番禺|马戏|Circus|中国|中國/i.test(s))
                return false;
              return /广州|廣州|Guangzhou|长隆|長隆|Chimelong|番禺|马戏|馬戲|Circus|서커스|中国|中國/i.test(s);
            }) || null;
          if (hit) {
            hit.click();
...
async function regs(page) {
  ...
  try {
    ...
    if ((await writeInc.count()) > 0) {
      ...
      if ((await desc.count()) > 0) await desc.fill('침롱 국제서커스 1일 입장권').catch(() => {});
      await page.evaluate(() => {
...
async function options(page) {
  if (!PRICE) {
    ...
    console.log('请设置: CHIMELONG_CIRCUS_PRICE=<表显示> node list-chimelong-circus-ticket.mjs options');
    process.exit(2);
  }
  const id = readDraft();
  if (!id) throw new Error('no draft');
