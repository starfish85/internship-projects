...
const DRAFT = process.env.KPK_DRAFT || readFileSync(join(__dir, '.kpk-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/kobe-port-kyoto';
const IMAGES = ['kpk-1.jpg', 'kpk-2.jpg', 'kpk-3.jpg']
...
const HIGHLIGHT = [
  `${CITY}과 ${DEST}를 편안하게 연결하는 단독 차량 이동`,
  '선박 승하선 일정에 맞춘 프라이빗 픽업 및 샌딩',
  '7인승 최대 4인+수하물 5개 · 10인승 최대 9인+수하물 10개',
].join('\n');
const INTRO = `이 서비스는 ${CITY}(고베항 / Kobe Port)와 ${DEST}(교토 시내 호텔·숙소) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 고베항 승하선 터미널 픽업/샌딩 장소, 선박명 및 승하선 정보, 교토 시내 호텔명·주소, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

크루즈·페리 이용 후 교토로 이동하시거나, 교토 관광 후 고베항으로 이동하시는 분들께 적합합니다.`;
...
2.미팅 장소: 고베항 픽업/샌딩 장소·선박명 및 승하선 정보와 교토 시내 호텔 주소·이용 시간을 정확히 입력해 주세요.
...
3.예약 시 호텔 주소, 교토 시내 픽업/샌딩 장소, 선박명·승하선 시간, 인원 및 수하물을 정확히 입력해 주세요.`;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
console.log('【将要】介绍页文案 + 썸네일 3 图');
killPeerCdpScripts('kpk-intro');
...
const textOk = await page.evaluate(() => ({
  hl: /고베|교토|Kobe|Kyoto/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /고베|교토|Kobe|Kyoto|선박|Port/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Kobe Cruise-Tokyo Stn');
