 ...
 * CSV 国内景区门票: 两套餐 · 只卖成人
 * 【售价来源】一般 125 / 特别展 305（表显示；加价逻辑 87.3 / 213.4 未用；未自算）
 * 双选项：일반 입장권 (갤러리 1-7) / 특별전 입장권 (갤러리 1-7 · 9)
 * 0n 双门禁：代表 电话+邮箱 + 按数量 英文姓·名·护照
 * Never 提交审核
 *
 * usage: node list-hk-palace-museum-ticket.mjs [bootstrap|attrs|intro|regs|options]
 */
...
const DRAFT_FILE = join(__dir, '.hk-palace-museum-draft-id');
const PHASE = process.argv[2] || 'bootstrap';
// 【售价来源】CSV 表显示；加价逻辑未用；未自算
const PRICE_GENERAL = '125';
const PRICE_SPECIAL = '305';
const PRICE = PRICE_GENERAL;
const PRODUCT_KO = '홍콩 고궁문화박물관 입장권';
const INTERNAL = 'Hong Kong Palace Museum Ticket';
const IMG_DIR = '/Users/mac/nol/upload-ready-images/hk-palace-museum';
// 0m：N=3 完整原图 hpm-1/2/3.jpg；禁裁切
const IMAGES = ['hpm-1.jpg', 'hpm-2.jpg', 'hpm-3.jpg']
  .map((f) => path.join(IMG_DIR, f))
  .filter((f) => fs.existsSync(f));

const HEADLINE = '홍콩 서구문화구에서 고궁 보물과 특별전을 만나보세요!';
const HIGHLIGHT = [
  '일반 입장권(갤러리 1-7) · 특별전 입장권(갤러리 1-7·9) 선택',
  '특별전권은 갤러리 9 고대 이집트전 포함 · 갤러리 8 미포함',
  '성인 전용 · 이메일 전자티켓/이용 안내',
].join('\n');
const INTRO = `홍콩 고궁문화박물관(Hong Kong Palace Museum) 입장권입니다.
서구문화구(West Kowloon)에서 고궁 컬렉션 테마 전시와 특별전을 관람할 수 있습니다. 본 상품은 두 가지 패키지를 판매합니다.
...
패키지 안내:
【일반 입장권 (갤러리 1-7)】
테마 전시 갤러리 1~7 입장. 특별전 갤러리 8·9는 포함되지 않습니다.

【특별전 입장권 (갤러리 1-7 · 9)】
테마 전시 갤러리 1~7 + 특별전 갤러리 9(고대 이집트 유물전, Ancient Egypt Unveiled) 입장.
갤러리 8 특별전은 포함되지 않습니다.
...
홍콩에서 고궁 문화와 특별전을 함께 보고 싶은 분들께 추천합니다.`;
...
3. 【일반 입장권】 갤러리 1~7 테마 전시만 포함합니다. 특별전 갤러리 8·9는 입장할 수 없습니다.
   【특별전 입장권】 갤러리 1~7 + 갤러리 9 고대 이집트 유물전을 포함합니다. 갤러리 8은 포함되지 않습니다.
...
2. 안내에 따라 지정 입구·시간에 맞춰 입장합니다. 예약한 패키지에 포함된 갤러리만 이용할 수 있습니다.
...
예약 시 영문 성명·여권번호 필수`,
    ...
    ptd: '성인 전용 · 만 19세 이상',
    price: PRICE_GENERAL,
    includeDesc: '고궁문화박물관 테마 전시 갤러리 1-7 입장',
  },
  {
    key: 'special',
    name: '특별전 입장권 (갤러리 1-7 · 9)',
    desc: `홍콩 고궁문화박물관 특별전 입장권
포함: 테마 전시 갤러리 1~7 + 특별전 갤러리 9(고대 이집트 유물전)
미포함: 갤러리 8
성인 전용
이메일 전자티켓/이용 안내 기준 입장
예약 시 영문 성명·여권번호 필수`,
    ...
    ptd: '성인 전용 · 만 19세 이상',
    price: PRICE_SPECIAL,
    includeDesc: '고궁문화박물관 갤러리 1-7 + 갤러리 9 고대 이집트전 입장',
  },
];
const OPT = OPTIONS[0];
...
function readDraft() {
  if (process.env.MACAU_TOWER_DRAFT) return process.env.MACAU_TOWER_DRAFT.trim();
...
async function getPage() {
  ...
  if (w2 < 1280) {
    console.log('【结果】FAIL innerWidth', w2);
    process.exit(2);
  }
  console.log('【读回】innerWidth', w2);
  return { browser, page };
}
...
async function bootstrap(page) {
  console.log('【售价来源】一般=125 / 特别展=305（加价逻辑=87.3/213.4 未用作售价；未自算）');
  ...
  if (/고궁문화박물관|Palace Museum|故宫文化博物馆|故宮文化博物館|홍콩 고궁/i.test(body) && !/Macau Tower|마카오 타워|TeamLab|팀랩/i.test(body)) {
    console.log('Resume existing draft from list');
    const card = page
      .locator('div[class*="slot___StyledContainer4"]')
      .filter({ hasText: /고궁문화박물관|Palace Museum|故宫文化博物馆|故宮文化博物館/i })
      .first();
...
async function attrsState(page) {
  return page.evaluate(() => {
    ...
    const hasPoi = /香港|Hong Kong|홍콩|故宫|故宮|Palace|고궁|West Kowloon|서구/i.test(t)
...
async function attrs(page) {
  ...
  if (!st.hasTheme) {
    ...
    if ((await themeBtn.count()) > 0) {
      ...
      let picked = false;
      // 真选中：label[for=categoryIds_*] checked
      picked = await page.evaluate(() => {
        const dlg = document.querySelector('[role=dialog]');
        if (!dlg) return false;
        const want = ['景点', '景點', '旅游地', '旅遊地', '展览', '展覽', '观光', '觀光'];
        for (const name of want) {
          const lab = Array.from(dlg.querySelectorAll('label')).find((l) => (l.innerText || '').trim() === name);
          if (!lab) continue;
          const forId = lab.getAttribute('for');
          const inp = forId ? document.getElementById(forId) : null;
          if (inp && !inp.checked) lab.click();
          else if (!inp) lab.click();
          if (inp?.checked || true) return name;
        }
        return false;
      });
      if (picked) console.log('theme', picked);
  ...
  st = await attrsState(page);
  if (!st.hasPoi) {
    console.log('【将要】POI 香港故宫文化博物馆 / Hong Kong Palace Museum');
    ...
    if ((await addPoi.count()) > 0) {
      ...
      const queries = ['Hong Kong Palace Museum', '香港故宫文化博物馆', '홍콩 고궁문화박물관', 'Hong Kong 故宫'];
      let clicked = null;
      for (const q of queries) {
        ...
        clicked = await page.evaluate(() => {
          ...
          const hit =
            items.find((e) => {
              const s = e.innerText || '';
              if (/한국|Korea|首尔|서울|호텔|Hotel|酒店/i.test(s) && !/Palace|故宫|故宮|고궁|West Kowloon|西九|香港|Hong Kong/i.test(s))
                return false;
              return /Palace Museum|故宫|故宮|고궁문화|West Kowloon|西九|Hong Kong|香港/i.test(s);
            }) || null;
          if (hit) {
            hit.click();
...
async function intro(page) {
  ...
  console.log('【将要】介绍文案 + 图 N=', IMAGES.length);
  await page.evaluate(
...
async function regs(page) {
  ...
  try {
    ...
    if ((await writeInc.count()) > 0) {
      ...
      if ((await desc.count()) > 0) await desc.fill('캔톤타워 450M 전망대 1일 입장권').catch(() => {});
      await page.evaluate(() => {
  ...
  // 0n 双门禁：按数量必做（表「无需护照」≠跳过按数量 UI；skill 2026-08-14）
  console.log('【将要】按数量 英文姓名+护照（0n）');
  await closeSheets();
...
/** P12 POST option 1段 + ETC + productType + version */
async function postOptionApi(page, productId, version, opt, salePrice) {
  ...
  const body = {
    ...
    attribute: {
      inclusions: [{ type: 'FACILITY_ADMISSION_FEE', description: '마카오 타워 전망대 1일 입장권' }],
      exclusions: [{ type: 'ETC', description: '개인 경비\n포함 사항에 명시되지 않은 기타 비용' }],
    },
    sale: { type: 'BY_RANGE', saleable: true, periods: [{ startDate, endDate, saleable: true, closed: false }] },
    prices: [{
      type: 'ADULT', name: '成人', description: opt.ptd || '성인 전용 · 만 19세 이상', representative: true,
      rule: { saleRule: { saleable: true }, bookingRule: { minimumPurchaseQuantityPerSession: 1, maximumPurchaseQuantityPerSession: 10, required: false, standalone: true, minPax: 1, maxPax: 10 } },
      policy: { type: 'BY_RANGE', periods: [{ startDate, endDate, salePrice: Number(salePrice) }] },
      addOnIds: [],
    }],
    rule: {
      useRule: { useType: 'USE_DATE' },
      bookingRule: { minimumPurchaseQuantityPerSession: 1, maximumPurchaseQuantityPerSession: 10, voucherAutoIssuable: true, voucherAutoIssuanceUnit: 'OPTION', voucherAutoIssuanceUsesDate: false },
    },
    timeslots: [],
  };
  return page.evaluate(async ({ productId, body }) => {
...
async function options(page) {
  if (!PRICE) {
    ...
    console.log('请设置: MACAU_TOWER_PRICE=<表显示> node list-canton-tower-450-ticket.mjs options');
    process.exit(2);
  }
  const id = readDraft();
  if (!id) throw new Error('no draft');
  ...
  // P12 API 优先（单选项）
  console.log('【将要】P12 API 落选项 价=', PRICE);
  const meta = await page.evaluate(async (pid) => {
    ...
    return { version: j.version || j.data?.version, names: (j.options || j.data?.options || []).map((o) => o.name), n: (j.options || j.data?.options || []).length };
  }, id);
  console.log('【读回】product', meta);
  if (meta.n >= 1 && meta.names.some((n) => /1일|입장권/.test(n))) {
    console.log('【结果】API 已有选项 STOP');
    return true;
  }
  const res = await postOptionApi(page, id, meta.version, OPT, PRICE);
  console.log('【读回】POST option', res);
  if (res.status >= 200 && res.status < 300) {
    await page.goto(LIST, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await sleep(2500);
    const final = await page.evaluate(async (pid) => {
      const r = await fetch(`/product-management/api/v3/temporary-products/${pid}`, { credentials: 'include' });
      const j = await r.json();
      const opts = j.options || j.data?.options || [];
      return { n: opts.length, opts: opts.map((o) => ({ name: o.name, price: o.prices?.[0]?.policy?.periods?.[0]?.salePrice })) };
    }, id);
    console.log('【结果】FINAL api', final);
    console.log('STOP 选项管理 — never 提交审核');
    if (final.n < 1 || !final.opts.every((o) => Number(o.price) === Number(PRICE))) process.exit(2);
    return true;
  }
  console.log('API 失败，回退 UI 路径', res);
  ...
  if ((await fac.count()) > 0) {
    ...
    if ((await d.count()) > 0) await d.fill('마카오 타워 전망대 1일 입장권');
  }
  // 排除必填，否则红字「包含和不包含…」
...
console.log('【复用入口】门票 skill P0–P10/P12 + list-hk-palace-museum-ticket 流程壳 + 售价列 0h');
console.log('【售价来源】一般=125 / 特别展=305（表显示；加价逻辑=87.3/213.4 未用作售价；未自算）');
console.log('【图片】N=' + IMAGES.length + ' 来源=路径 hk-palace-museum');
