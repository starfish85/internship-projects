...
const DRAFT = process.env.NTD_DRAFT || readFileSync(join(__dir, '.ntd-draft-id'), 'utf8').trim();
...
const IMG_DIR = '/Users/mac/nol/upload-ready-images/narita-disney';
const IMAGES = ['ntd-1.jpg', 'ntd-2.jpg', 'ntd-3.jpg'].map((f) => path.join(IMG_DIR, f)).filter(fs.existsSync);
const A = '나리타 공항(NRT)';
const B = '도쿄 디즈니 리조트';
...
const INTRO = `이 서비스는 ${A}(Narita International Airport)와 ${B}(Tokyo Disney Resort) 사이의 편도 전용 차량 이동 서비스입니다.
...
예약 시 항공편명·도착/출발 시간·터미널, 도쿄 디즈니 리조트 픽업/샌딩 게이트 또는 호텔, 이용 시간, 연락 가능한 휴대전화 번호, 탑승 인원, 수하물 수량을 정확히 입력해 주세요.
기사님이 보통 이용일 하루 전 WhatsApp/SMS로 연락드리니, 번호가 WhatsApp에 등록되어 있고 계정이 정상인지 확인해 주세요.

나리타 공항 출도착 후 디즈니랜드·디즈니시로 바로 이동하시거나, 파크 이용 후 공항으로 이동하시는 분들께 적합합니다.`;
...
const textOk = await page.evaluate(() => ({
  hl: /나리타|NRT|디즈니|Disney|Narita/i.test(document.querySelector('#headline, input[name=headline]')?.value || ''),
  desc: /나리타|NRT|디즈니|Disney|Narita|Tokyo/i.test(document.querySelector('#description, textarea[name=description]')?.value || ''),
}));
console.log('【读回】文案', textOk);
if (!textOk.hl) failExit('headline missing Narita-Disney');
