...
def run_scrape(
    ...
    if only_ticket and only_ticket.lower() == "under 4":
        if not collected:
            raise RuntimeError("Under 4 is free and cannot be selected separately. Collect a paid ticket first.")
        log("Under 4 is free; reusing existing calendar rows and marking price as 0.")
        synthesized = synthesize_free_ticket_rows(collected, target_month, age_group="Under 4")
    ...
    with sync_playwright() as pw:
        ...
        try:
            for ticket in ticket_types:
                ...
                if age_group.lower() == "under 4":
                    log("Under 4 is free; reusing existing calendar rows and marking price as 0.")
                    synthesized = synthesize_free_ticket_rows(collected, target_month, age_group="Under 4")
                    if not synthesized:
                        log("No source rows found to synthesize Under 4.")
                    for row in synthesized:
                        append_unique_row(collected, row)
                    write_outputs(filter_rows_by_month(collected, target_month), output_dir)
                    continue
...
def write_outputs(rows: list[TicketRow], output_dir: Path) -> tuple[Path, Path]:
    ...
    try:
        ...
        csv_path = output_dir / f"ticket_availability_{fallback_stamp}.csv"
        json_path = output_dir / f"ticket_availability_{fallback_stamp}.json"
        csv_file = csv_path.open("w", newline="", encoding="utf-8-sig")
        ...
        for item in reader:
            rows.append(
                TicketRow(
                    ...
                    currency=item.get("currency", "JPY") or "JPY",
                )
            )
    return rows
...
def synthesize_free_ticket_rows(
    rows: list[TicketRow],
    target_month: str | None,
    age_group: str = "Under 4",
) -> list[TicketRow]:
    source_rows = [
        row
        for row in rows
        if row.time_slot
        and row.price
        and row.age_group.lower() in {"adult", "junior", "child"}
        and (not target_month or row.date.startswith(target_month))
    ]
    if not source_rows:
        return []

    preferred_order = {"adult": 0, "junior": 1, "child": 2}
    source_rows.sort(key=lambda row: (row.date, row.time_slot, preferred_order.get(row.age_group.lower(), 99)))

    synthesized: list[TicketRow] = []
    ...
    for row in source_rows:
        ...
        synthesized.append(
            TicketRow(
                ...
                age_group=age_group,
                price="0",
                sale_status=row.sale_status or "Available",
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", help="Output directory. Defaults to a timestamped folder under generated/runs.")
    ...
    if args.output_dir:
        ...
        output_dir = (RUNS_ROOT / stamp).resolve()
